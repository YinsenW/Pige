...
interface DestinationReconnectContext {
  readonly vaultId: string;
...
interface BackupDestinationReconnectServiceOptions {
  readonly capture: (jobId: string) => DestinationReconnectContext;
  readonly now: () => Date;
  readonly onQueued: (context: DestinationReconnectContext, queued: JobRecordSnapshot) => void;
}

export class BackupDestinationReconnectService {
  readonly #options: BackupDestinationReconnectServiceOptions;

  constructor(options: BackupDestinationReconnectServiceOptions) {
  ...
  inspect(
    ...
    const context = this.#options.capture(jobId);
  ...
  reconnect(
    ...
    const inspected = this.inspect(candidate.vaultId, candidate.jobId, candidate.jobUpdatedAt);
    if (inspected.status !== "ready") return inspected.status;
    const context = this.#options.capture(candidate.jobId);
    try {
      ...
      this.#options.onQueued(context, queued);
      return "reconnected";
    } catch (caught) {
      if (caught instanceof PigeDomainError && new Set([
        "backup.destination_reconnect_stale",
        "job.claim_conflict",
        "job.claim_lost",
        "job.revision_conflict"
      ]).has(caught.code)) return "stale";
...
export function settleBackupDestinationWait(
  ...
  return new JobExecutionCoordinator(store, { now: () => now }).settle(snapshot, {
    kind: "waiting",
    reason: "dependency",
