...
export interface CurrentNoteConflictResolution {
  ...
  readonly decision: "keep_current" | "apply_proposed" | "save_proposed_as_note";
...
export class CurrentNoteConflictReviewService {
  ...
  resolve(input: {
    ...
    if (
      ...
      (input.decision === "save_proposed_as_note") !== (input.createdPageId !== undefined) ||
...
function isResolution(value: unknown): value is ResolutionRecord {
  ...
  return record.schemaVersion === 1 && record.kind === "current_note_conflict_resolution" &&
    ...
    (record.decision === "keep_current" || record.decision === "apply_proposed" || record.decision === "save_proposed_as_note") &&
    (record.decision !== "keep_current") === (record.operationId !== undefined) &&
    (record.decision === "save_proposed_as_note") === (record.createdPageId !== undefined) &&
    (record.operationId === undefined || OperationIdSchema.safeParse(record.operationId).success) &&
    (record.createdPageId === undefined || /^page_\d{8}_[a-z0-9]{8,}$/u.test(record.createdPageId)) &&
...
function projectResolution(record: ResolutionRecord): CurrentNoteConflictResolution {
  return {
    ...
    ...(record.createdPageId ? { createdPageId: record.createdPageId } : {})
  };
}
