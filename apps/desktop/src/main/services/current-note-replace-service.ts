...
import { CurrentNoteConflictSaveService, currentNoteConflictSavedOperationMatches, readCurrentNoteConflictReviewBase } from "./current-note-conflict-save-service";
...
export interface CurrentNoteReplaceRequest {
  readonly vaultPath: string;
  readonly activeVaultId: string;
  readonly job: JobRecord;
  readonly inspection: CurrentNoteReplaceInspection;
  readonly modelProfileId: string;
  readonly markdown: string;
}

export interface CurrentNoteReplaceProposalPreview {
...
export type CurrentNoteReplaceResult =
  | {
  ...
  | {
      ...
      readonly proposal: CurrentNoteReplaceProposalPreview;
    };

export type CurrentNoteReplaceProposalDecisionResult =
  | { readonly status: "applied"; readonly proposal: CurrentNoteReplaceProposalPreview; readonly operation: OperationRecord; readonly createdPageId?: string }
  | { readonly status: "rejected" | "conflicted"; readonly proposal: CurrentNoteReplaceProposalPreview }
  | { readonly status: "stale"; readonly proposal?: CurrentNoteReplaceProposalPreview }
  | { readonly status: "not_found" };

export type CurrentNoteReplacePublicationStatus =
...
interface ReplaceIntentRecord {
  readonly schemaVersion: 1;
  readonly kind: "current_note_replace_intent";
  readonly activeVaultId: string;
  readonly jobId: string;
...
interface ReplaceProposalRecord {
  readonly schemaVersion: 1;
  readonly kind: "current_note_replace_proposal";
  readonly proposalId: string;
  ...
  readonly createdAt: string;
  readonly preview: CurrentNoteReplaceProposalPreview["lines"];
}
interface ProposalDecisionRecord {
  readonly schemaVersion: 1;
  readonly kind: "current_note_replace_decision";
  readonly proposalId: string;
  readonly intentHash: string;
...
interface ProposalOutcomeRecord {
  readonly schemaVersion: 1;
  readonly kind: "current_note_replace_outcome";
  readonly proposalId: string;
  readonly intentHash: string;
...
interface ReplaceReceiptRecord {
  readonly schemaVersion: 1;
  readonly kind: "current_note_replace_receipt";
  readonly intentHash: string;
  readonly operationId: string;
  readonly afterHash: string;
  readonly committedAt: string;
}
export class CurrentNoteReplaceService {
  publish(request: CurrentNoteReplaceRequest): CurrentNoteReplacePublicationStatus {
    const result = this.replace(request);
    return result.status === "committed"
      ? { status: "applied", operationId: result.operation.id }
      : { status: "review_required", proposalId: result.proposal.proposalId };
  }
  replace(request: CurrentNoteReplaceRequest): CurrentNoteReplaceResult {
    const job = assertReplaceJob(request);
    ...
    persistExactJson(request.vaultPath, jobBindingPath(job.id), {
      schemaVersion: 1,
      kind: "current_note_replace_job_binding",
      jobId: job.id,
    ...
    if (stableJson(durableIntent) !== stableJson(intent)) throw turnConflict("The current-note replacement Job is already bound to another immutable intent.");

    const adopted = this.#adoptCommitted(request.vaultPath, durableIntent, intentHash);
    if (adopted) return adopted;
    const proposalId = createProposalId(job.id, durableIntent.pageId);
    const existingProposal = readProposal(request.vaultPath, proposalId);
    if (existingProposal) {
      if (existingProposal.intentHash !== intentHash) {
        throw turnConflict("The current-note replacement proposal changed immutable intent.");
      }
      return {
        status: "awaiting_review",
        proposal: this.#preview(request.vaultPath, existingProposal, durableIntent)
      };
    }
    const current = readCurrentTarget(request.vaultPath, durableIntent);
    const proposal = this.#stageProposal(
      request.vaultPath,
      durableIntent,
      intentHash,
      current.markdown,
  ...
  recover(request: CurrentNoteReplaceRequest): CurrentNoteReplaceResult {
    const result = this.replace(request);
    if (result.status !== "awaiting_review" || result.proposal.state !== "resolving") return result;
    const decision = this.decideProposal({
    ...
    if (decision.status === "applied") {
      return { status: "committed", pageId: result.proposal.pageId, operation: decision.operation, recovered: true };
    }
    if (decision.status === "not_found") {
      throw turnConflict("The durable current-note replacement proposal disappeared during recovery.");
    }
    return { status: "awaiting_review", proposal: decision.proposal ?? result.proposal };
  }
  readPublication(input: {
    readonly vaultPath: string;
    readonly activeVaultId: string;
    readonly job: JobRecord;
  }): CurrentNoteReplacePublicationStatus | undefined {
    ...
    if (binding.jobId !== job.id) throw turnConflict("The current-note replacement Job binding changed identity.");
    ...
    if (
      ...
      throw turnConflict("The current-note replacement publication does not match its durable Job binding.");
    }
    const intentHash = hashJson(intent);
    ...
    if (!proposal) {
      if ((job.operationIds ?? []).includes(intent.operationId) || (job.proposalIds?.length ?? 0) > 0) {
        throw turnConflict("The current-note replacement publication referenced by the Job is unavailable.");
      }
      return undefined;
    }
    if (proposal.intentHash !== intentHash) throw turnConflict("The current-note replacement proposal changed immutable intent.");
  ...
  getProposal(input: {
    ...
    readonly jobId: string;
    readonly proposalId: string;
  }): CurrentNoteReplaceProposalPreview | undefined {
  ...
  get(input: Parameters<CurrentNoteReplaceService["getProposal"]>[0]): CurrentNoteReplaceProposalPreview | undefined {
    return this.getProposal(input);
  }

  decide(input: Parameters<CurrentNoteReplaceService["decideProposal"]>[0]): CurrentNoteReplacePublicationStatus {
    const result = this.decideProposal(input);
    if (result.status === "not_found") throw turnConflict("The current-note replacement proposal is unavailable.");
    if (result.status === "stale" && !result.proposal) {
      throw turnConflict("The current-note replacement proposal no longer has a durable identity.");
    }
  ...
  decideProposal(input: {
    ...
    readonly decision: "approve" | "reject" | "keep_current" | "apply_proposed" | "save_proposed_as_new_page";
    ...
    if (
      ...
      throw turnConflict("The current-note replacement proposal no longer matches its immutable Job, page, and intent binding.");
    }
    ...
    if (existingOutcome) {
      const resolvingConflict = input.decision === "keep_current" || input.decision === "apply_proposed" || input.decision === "save_proposed_as_new_page";
      ...
      if (input.decision === "save_proposed_as_new_page") {
        if (input.expectedRevision !== 3 || !input.expectedCurrentRevision) return { status: "stale", proposal: this.#preview(input.vaultPath, proposal, intent, existingOutcome) };
        const reviewBase = readCurrentNoteConflictReviewBase(input.vaultPath, conflictReviewBasePath(proposal.proposalId), proposal.reviewBaseHash);
        const parsedBase = parsePigeFrontmatter(reviewBase);
        if (!parsedBase) throw turnConflict("The replacement conflict review base is invalid.");
        const saved = conflictSaves.resolve({
          vaultPath: input.vaultPath, mutationKind: "replace" as const, proposalId: proposal.proposalId,
          intentHash, jobId: intent.jobId, createdAt: intent.createdAt, sourcePageId: intent.pageId,
          sourceTitle: parsedBase.frontmatter.title ?? "Untitled note", body: `${intent.markdown.trim()}\n\nEvidence: [citation_1]`,
          modelProfileId: intent.modelProfileId, policyContextId: intent.policyContextId, policyHash: intent.policyHash,
          currentRevision: input.expectedCurrentRevision as `noteeditrev_${string}`, expectedRevision: input.expectedRevision,
          expectedCurrentRevision: input.expectedCurrentRevision,
          readPreview: () => this.#preview(input.vaultPath, proposal, intent, existingOutcome)
        });
        if (!saved) return { status: "stale", proposal: this.#preview(input.vaultPath, proposal, intent, existingOutcome) };
        return this.#decisionResult(input.vaultPath, proposal, intent, existingOutcome);
      }
    ...
    if (input.decision === "keep_current" || input.decision === "apply_proposed" || input.decision === "save_proposed_as_new_page") return { status: "stale", proposal: this.#preview(input.vaultPath, proposal, intent) };
    ...
    if (!existingDecision) {
      ...
      persistExactJson(input.vaultPath, decisionPath(proposal.proposalId), {
        schemaVersion: 1, kind: "current_note_replace_decision",
    ...
    if (durableDecision.decision !== input.decision || durableDecision.intentHash !== intentHash) throw turnConflict("The current-note replacement proposal already has another durable decision.");
    ...
    if (committed.status !== "committed") throw turnConflict("The approved current-note replacement did not reach a durable Operation.");
  ...
  #adoptCommitted(vaultPath: string, intent: ReplaceIntentRecord, intentHash: string): Extract<CurrentNoteReplaceResult, { status: "committed" }> | undefined {
    ...
    if (!binding) throw turnConflict("The current-note replacement Operation is not compatible with Activity and Undo.");
    const current = readCurrentTarget(vaultPath, intent);
    if (current.contentHash !== binding.afterHash) throw turnConflict("The current note changed after its durable replacement Operation.");
  ...
  #adoptPageEffect(
    vaultPath: string,
    intent: ReplaceIntentRecord,
    intentHash: string,
    liveHash: string,
    expectedBeforeHash: string
  ): Extract<CurrentNoteReplaceResult, { status: "committed" }> | undefined {
    ...
    const expectedAfterHash = hashText(createReplacementMarkdown(before, intent));
  ...
  #commit(
    ...
    beforeHash: string,
    recovered: boolean
  ): Extract<CurrentNoteReplaceResult, { status: "committed" }> {
    const next = createReplacementMarkdown(before, intent);
    ...
    if (live.contentHash === beforeHash) {
      ...
      throw turnConflict("The current note changed before its replacement could commit.");
    }
  ...
  #stageProposal(vaultPath: string, intent: ReplaceIntentRecord, intentHash: string, current: string, currentHash: string): ReplaceProposalRecord {
    const proposal: ReplaceProposalRecord = {
      schemaVersion: 1,
      kind: "current_note_replace_proposal",
    ...
    stageExact(vaultPath, conflictReviewBasePath(proposal.proposalId), current, currentHash);
    ...
    if (!durable || stableJson(durable) !== stableJson(proposal)) throw turnConflict("The current-note replacement proposal identity is occupied by another review.");
    return durable;
  }
  ...
  #preview(vaultPath: string, proposal: ReplaceProposalRecord, intent: ReplaceIntentRecord, knownOutcome?: ProposalOutcomeRecord): CurrentNoteReplaceProposalPreview {
    ...
    if (resolution) {
      return {
        ...
        state: resolution.decision === "keep_current" ? "rejected" : "applied",
        revision: 4,
        activeVaultId: intent.activeVaultId,
        pageId: intent.pageId,
        jobId: intent.jobId,
  ...
  #decisionResult(vaultPath: string, proposal: ReplaceProposalRecord, intent: ReplaceIntentRecord, outcome: ProposalOutcomeRecord): CurrentNoteReplaceProposalDecisionResult {
    ...
    if (outcome.outcome === "applied" || preview.state === "applied") {
      ...
      const operationId = outcome.operationId ?? resolution?.operationId;
      const operation = operationId ? readOperation(vaultPath, operationId) : undefined;
      if (!operation) throw turnConflict("The applied replacement proposal is missing its exact Operation.");
      if (resolution?.decision === "save_proposed_as_new_page") {
        if (!currentNoteConflictSavedOperationMatches(operation, intent, proposal.proposalId, resolution.pageId)) throw turnConflict("The saved replacement proposal Operation changed immutable identity.");
      } else assertOperationMatchesIntent(operation, intent);
      return { status: "applied", proposal: preview, operation, ...(resolution?.pageId ? { createdPageId: resolution.pageId } : {}) };
    }
    return { status: outcome.outcome, proposal: preview };
  }
}

function assertReplaceJob(request: CurrentNoteReplaceRequest): JobRecord {
  ...
  if (
    ...
    throw new PigeDomainError("agent_runtime.turn_binding_invalid", "The current-note replacement is not bound to one successful current-note inspection.");
  }
  return job;
}

function createIntent(request: CurrentNoteReplaceRequest, job: JobRecord): ReplaceIntentRecord {
  assertReplacementMarkdown(request.markdown);
  if (!PageIdSchema.safeParse(request.inspection.pageId).success || !isContentHash(request.inspection.contentHash) || !isContentHash(request.inspection.bindingHash)) {
    throw new PigeDomainError("agent_runtime.tool_input_invalid", "The current-note replacement inspection identity is invalid.");
  }
  if (!/^model_[a-z0-9_]+$/u.test(request.modelProfileId)) throw new PigeDomainError("agent_runtime.tool_input_invalid", "The current-note replacement model identity is invalid.");
  ...
  const artifactChecksum = hashJson({
    kind: "current_note_replace_evidence",
    jobId: job.id,
  ...
  return {
    schemaVersion: 1,
    kind: "current_note_replace_intent",
    activeVaultId: request.activeVaultId,
    jobId: job.id,
...
function readCurrentTarget(vaultPath: string, intent: Pick<ReplaceIntentRecord, "pageId" | "pagePath">): { readonly markdown: string; readonly contentHash: string } {
  ...
  if (target.item.summary.pagePath !== intent.pagePath || target.page.contentHash !== hashText(target.markdown)) {
    throw turnConflict("The current-note replacement target path or durable bytes changed identity.");
  }
  return { markdown: target.markdown, contentHash: target.page.contentHash };
}

function createReplacementMarkdown(before: string, intent: ReplaceIntentRecord): string {
  if (hashText(intent.markdown) !== intent.markdownHash) throw turnConflict("The current-note replacement text no longer matches its immutable intent.");
  const parsed = parsePigeFrontmatter(before);
  if (!parsed || parsed.frontmatter.id !== intent.pageId || parsed.frontmatter.type !== "note") throw turnConflict("The current-note replacement target is not the exact supported note page.");
  ...
  if (Buffer.byteLength(result, "utf8") > MAX_PAGE_BYTES || containsRestrictedModelContent(result)) throw new PigeDomainError("agent_runtime.tool_input_invalid", "The current-note replacement would create an unsafe or oversized note.");
  return result;
}

function createOperation(intent: ReplaceIntentRecord, beforeHash: string, afterHash: string): OperationRecord {
  return OperationRecordSchema.parse({
    ...
    policyAudit: {
      ...
      enforcementOwners: ["Home Agent Service", "Current Note Replace Service"]
    },
    kind: "update_page",
    ...
    summary: `Replaced the evidence-bound body of current note ${intent.pageId}.`,
    reversible: "yes",
...
function assertOperationMatchesIntent(operation: OperationRecord, intent: ReplaceIntentRecord): void {
  if (
  ...
  ) throw turnConflict("The current-note replacement Operation does not match its immutable intent.");
}

function assertExactOperation(actual: OperationRecord, expected: OperationRecord): void {
  if (stableJson(actual) !== stableJson(expected)) throw turnConflict("The current-note replacement Operation identity is occupied by different facts.");
}

function assertReplacementMarkdown(value: string): void {
  const bytes = Buffer.byteLength(value, "utf8");
  if (
    bytes < 1 ||
    bytes > MAX_REPLACEMENT_TEXT_BYTES ||
    value.trim().length === 0 ||
    /[\u0000\u000b\u000c\u007f]/u.test(value) ||
    /<!--\s*pige:managed:/iu.test(value) ||
    /\[(?:citation_(?!1\])[a-z0-9_]+|source:[^\]]+)\]/iu.test(value) ||
    containsRestrictedModelContent(value)
  ) throw new PigeDomainError("agent_runtime.tool_input_invalid", "The current-note replacement text is unsafe, restricted, or outside its bound.");
}

function createSafePreview(current: string, replacement: string): CurrentNoteReplaceProposalPreview["lines"] {
  const context = current.split(/\r?\n/u).map((line) => line.trim()).filter(Boolean).at(-1) ?? "Current note";
  return [
    { kind: "removed", text: truncateSafe(context, 160) },
    { kind: "added", text: truncateSafe(replacement.replace(/\s+/gu, " ").trim(), 160) }
  ];
}
...
function persistReceipt(vaultPath: string, intent: ReplaceIntentRecord, intentHash: string, operation: OperationRecord, recovered: boolean): void {
  const binding = readAgentPageUpdateOperationBinding(operation);
  if (!binding) throw turnConflict("The current-note replacement receipt cannot bind an invalid Operation.");
  const receipt: ReplaceReceiptRecord = {
    schemaVersion: 1,
    kind: "current_note_replace_receipt",
    intentHash,
...
function commitOperation(vaultPath: string, operation: OperationRecord): OperationRecord {
  ...
  if (!committed) throw turnConflict("The current-note replacement Operation could not be adopted.");
...
function stageExact(vaultPath: string, relativePath: string, value: string, expectedHash: string): void {
  if (hashText(value) !== expectedHash) throw turnConflict("The current-note replacement staging checksum is invalid.");
  ...
  if (result === "exists") {
    ...
    if (existing === undefined || hashText(existing) !== expectedHash) throw turnConflict("The current-note replacement staging identity is occupied by different bytes.");
  }
}
...
function persistExactJson(
  ...
  if (result === "exists") {
    ...
    if (existing === undefined) throw turnConflict("A durable current-note replacement record became unavailable.");
    let parsed: unknown;
    try { parsed = JSON.parse(existing); } catch { throw turnConflict("A durable current-note replacement record is invalid."); }
    if (identity(parsed) !== identity(value)) throw turnConflict("A durable current-note replacement record identity is occupied by different facts.");
  }
}
...
function readJson(vaultPath: string, relativePath: string): unknown | undefined {
  ...
  try { return JSON.parse(content); } catch { throw turnConflict("A durable current-note replacement record is invalid."); }
}

function readRequiredIntent(vaultPath: string, operationId: string): ReplaceIntentRecord {
  const value = readJson(vaultPath, intentPath(operationId));
  if (!isIntent(value)) throw turnConflict("The durable current-note replacement intent is unavailable or invalid.");
  return value;
}
...
function readJobBinding(vaultPath: string, jobId: string): {
  ...
  if (
    ...
    value.kind !== "current_note_replace_job_binding" ||
    typeof value.jobId !== "string" ||
    typeof value.pageId !== "string" ||
    typeof value.operationId !== "string"
  ) {
    throw turnConflict("The durable current-note replacement Job binding is invalid.");
  }
  return { jobId: value.jobId, pageId: value.pageId, operationId: value.operationId };
}

function readProposal(vaultPath: string, proposalId: string): ReplaceProposalRecord | undefined {
  const value = readJson(vaultPath, proposalPath(proposalId));
  if (value === undefined) return undefined;
  if (!isProposal(value)) throw turnConflict("The durable current-note replacement proposal is invalid.");
  return value;
}
...
function readDecision(vaultPath: string, proposalId: string): ProposalDecisionRecord | undefined {
  ...
  if (value === undefined) return undefined;
  if (!isDecision(value)) throw turnConflict("The durable current-note replacement decision is invalid.");
  return value;
}
...
function readRequiredDecision(vaultPath: string, proposalId: string): ProposalDecisionRecord {
  ...
  if (!decision) throw turnConflict("The durable current-note replacement decision is unavailable.");
  return decision;
}
...
function readOutcome(vaultPath: string, proposalId: string): ProposalOutcomeRecord | undefined {
  ...
  if (value === undefined) return undefined;
  if (!isOutcome(value)) throw turnConflict("The durable current-note replacement outcome is invalid.");
  return value;
}

function commitOutcome(vaultPath: string, proposal: ReplaceProposalRecord, intentHash: string, outcome: ProposalOutcomeRecord["outcome"], operationId?: string): ProposalOutcomeRecord {
  const record: ProposalOutcomeRecord = {
    schemaVersion: 1,
    kind: "current_note_replace_outcome",
    proposalId: proposal.proposalId,
    intentHash,
    outcome,
  ...
  if (!durable) throw turnConflict("The current-note replacement outcome could not be adopted.");
  return durable;
}

function isIntent(value: unknown): value is ReplaceIntentRecord {
  if (!isRecord(value)) return false;
  return value.schemaVersion === 1 && value.kind === "current_note_replace_intent" && typeof value.activeVaultId === "string" && typeof value.jobId === "string" && typeof value.createdAt === "string" && typeof value.pageId === "string" && typeof value.pagePath === "string" && typeof value.beforeHash === "string" && typeof value.inspectionBindingHash === "string" && Array.isArray(value.evidenceRefs) && value.evidenceRefs.length === 1 && value.evidenceRefs[0] === "citation_1" && typeof value.markdown === "string" && typeof value.markdownHash === "string" && typeof value.modelProfileId === "string" && typeof value.policyContextId === "string" && typeof value.policyHash === "string" && typeof value.operationId === "string" && typeof value.beforePath === "string" && typeof value.stagedPath === "string" && typeof value.operationPath === "string" && typeof value.artifactId === "string" && typeof value.artifactChecksum === "string";
}

function isProposal(value: unknown): value is ReplaceProposalRecord {
  return isRecord(value) && value.schemaVersion === 1 && value.kind === "current_note_replace_proposal" && typeof value.proposalId === "string" && typeof value.intentHash === "string" && typeof value.activeVaultId === "string" && typeof value.jobId === "string" && typeof value.pageId === "string" && typeof value.pagePath === "string" && typeof value.reviewBaseHash === "string" && typeof value.createdAt === "string" && Array.isArray(value.preview);
}

function isDecision(value: unknown): value is ProposalDecisionRecord {
  return isRecord(value) && value.schemaVersion === 1 && value.kind === "current_note_replace_decision" && typeof value.proposalId === "string" && typeof value.intentHash === "string" && (value.decision === "approve" || value.decision === "reject") && typeof value.decidedAt === "string";
}

function isOutcome(value: unknown): value is ProposalOutcomeRecord {
  return isRecord(value) && value.schemaVersion === 1 && value.kind === "current_note_replace_outcome" && typeof value.proposalId === "string" && typeof value.intentHash === "string" && ["applied", "rejected", "conflicted"].includes(String(value.outcome)) && typeof value.completedAt === "string";
}
...
function receiptIdentity(value: unknown): string {
  const record = value as Partial<ReplaceReceiptRecord>;
...
function monotonicTimestamp(current: string, requested: string): string {
  ...
  if (!Number.isFinite(currentTime) || !Number.isFinite(requestedTime)) throw turnConflict("The current-note replacement timestamp binding is invalid.");
...
function createArtifactId(jobId: string, pageId: string): string {
  return `art_current_note_replace_${digest(`artifact\0${jobId}\0${pageId}`).slice(0, 16)}`;
}
...
function privateRoot(operationId: string): string {
  ...
  return [".pige", "agent", "current-note-replaces", dateKey.slice(0, 4), dateKey.slice(4, 6)].join("/");
}
...
function jobBindingPath(jobId: string): string {
  const dateKey = /^job_(\d{8})_/u.exec(jobId)?.[1] ?? "19700101";
  return `.pige/agent/current-note-replaces/${dateKey.slice(0, 4)}/${dateKey.slice(4, 6)}/${jobId}.binding.json`;
}
function proposalPath(proposalId: string): string { return `.pige/agent/current-note-replace-proposals/${proposalId}.json`; }
function conflictReviewBasePath(proposalId: string): string { return `.pige/agent/current-note-replace-proposals/${proposalId}.review-base.md`; }
function decisionPath(proposalId: string): string { return `.pige/agent/current-note-replace-proposals/${proposalId}.decision.json`; }
function outcomePath(proposalId: string): string { return `.pige/agent/current-note-replace-proposals/${proposalId}.outcome.json`; }
...
function resolveVaultPath(vaultPath: string, relativePath: string): string {
  if (!relativePath || path.isAbsolute(relativePath) || relativePath.includes("\\") || relativePath.split("/").some((segment) => !segment || segment === "." || segment === "..")) throw turnConflict("The current-note replacement path is invalid.");
  const root = path.resolve(vaultPath);
  const rootStat = fs.lstatSync(root);
  if (!rootStat.isDirectory() || rootStat.isSymbolicLink()) throw turnConflict("The active vault is not a safe current-note replacement root.");
  const resolved = path.resolve(root, ...relativePath.split("/"));
  if (!resolved.startsWith(`${root}${path.sep}`)) throw turnConflict("The current-note replacement path escapes the active vault.");
  return resolved;
}
...
export const CURRENT_NOTE_REPLACE_TOOL = Object.freeze({ id: REPLACE_TOOL_ID, version: REPLACE_TOOL_VERSION });
