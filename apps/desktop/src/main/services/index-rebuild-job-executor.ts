...
export interface IndexRebuildJobExecutorPort {
  createJob(): JobRecord;
  queued(request: ProcessQueuedIndexRebuildRequest): readonly QueuedIndexRebuildJob[];
...
export class IndexRebuildJobExecutor {
  ...
  async request(): Promise<LocalDatabaseRebuildResult> {
    const job = this.#port.createJob();
    const result = await this.process({ jobIds: [job.id] });
  ...
  async process(
    ...
    const next = this.#tail.then(() => this.#process(request));
    this.#tail = next.then(() => undefined, () => undefined);
    return next;
  }

  async #process(request: ProcessQueuedIndexRebuildRequest): Promise<ProcessQueuedIndexRebuildResult> {
