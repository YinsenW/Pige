      ...
      const outgoingRows = db.prepare(
        `
          SELECT * FROM (
            ...
            WHERE e.from_page_id = ? AND e.relation_type IN ('related_to', 'mentions_entity', 'contradicts', 'supports', 'supersedes', 'answers', 'broader_than')
      ...
      const backlinkRows = db.prepare(
        `
          SELECT * FROM (
            ...
            WHERE e.to_page_id = ? AND e.relation_type IN ('related_to', 'mentions_entity', 'contradicts', 'supports', 'supersedes', 'answers', 'broader_than')
          ) ORDER BY updated_at DESC, page_path ASC, relation_type ASC
          LIMIT ?
        `
      ).all(request.pageId, request.pageId, limit);
      ...
      return {
        totalOutgoing: readCount(
          ...
          `SELECT COUNT(*) AS count FROM (
            ...
            UNION SELECT relation_type, to_page_id FROM relation_edges
              WHERE from_page_id = ? AND relation_type IN ('related_to', 'mentions_entity', 'contradicts', 'supports', 'supersedes', 'answers', 'broader_than')
        ...
        totalBacklinks: readCount(db, `SELECT COUNT(*) AS count FROM (
          ...
          UNION SELECT relation_type, from_page_id FROM relation_edges
            WHERE to_page_id = ? AND relation_type IN ('related_to', 'mentions_entity', 'contradicts', 'supports', 'supersedes', 'answers', 'broader_than')
  ...
  knowledgeTree(vaultPath: string): KnowledgeTreeSnapshot | undefined {
    ...
    try {
      ...
      const relations = db.prepare(`
        ...
        FROM relation_edges
        WHERE relation_type IN ('has_topic', 'links_to', 'mentions_entity', 'related_to', 'broader_than')
...
function rowToRelatedPage(row: Record<string, unknown>, relation: LibraryRelatedPage["relation"]): LibraryRelatedPage {
  ...
  return {
    ...
    relation,
    relationType: ["related_to", "mentions_entity", "contradicts", "supports", "supersedes", "answers", "broader_than"].includes(String(row.relation_type))
