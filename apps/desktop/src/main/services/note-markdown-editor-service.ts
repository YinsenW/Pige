...
interface NoteMarkdownEditorDependencies {
  ...
  readonly allowClaim?: boolean; readonly allowQuestion?: boolean; }
...
export class NoteMarkdownEditorService {
  ...
  constructor(
    ...
    this.#allowClaim = dependencies.allowClaim ?? false; this.#allowQuestion = dependencies.allowQuestion ?? false;
  }
  open(request: NoteMarkdownEditorOpenRequest): NoteMarkdownEditorOpenResult {
    ...
    try {
      ...
      if (!validateEditablePageMarkdown(content.markdown, request.pageId, this.#allowClaim, this.#allowQuestion)) return { status: "failed" };
  ...
  save(request: NoteMarkdownEditorSaveRequest, operationKind: "update_page" | "archive_page" | "restore_page" = "update_page"): NoteMarkdownEditorSaveResult {
    ...
    if (!isEditableMarkdownPageType(request.markdown, this.#allowClaim, this.#allowQuestion)) {
    ...
    if (!preservesEditableMarkdownPageOwnership(beforeMarkdown, request.markdown, this.#allowClaim, this.#allowQuestion)) {
      return { status: "invalid", ...identity, invalidReason: "unsupported_page_type" };
    }
  ...
  #replaceExact(binding: RenderBinding, beforeMarkdown: string, afterMarkdown: string): MarkdownFileSignatureRecord {
    ...
    try {
      ...
      if (committed !== afterMarkdown ||
        !validateEditablePageMarkdown(committed, binding.pageId, this.#allowClaim, this.#allowQuestion)) {
...
export class NoteMarkdownEditorActivityAdapter implements NoteMarkdownEditorActivityPort {
  ...
  recordPageUpdate(input: {
    ...
    if (
      ...
      !preservesEditableMarkdownPageOwnership(input.beforeMarkdown, input.afterMarkdown, true, true)
    ) {
...
function validateEditablePageMarkdown(markdown: string, expectedPageId: string, allowClaim: boolean, allowQuestion = false): boolean {
  return validatePortableMarkdown(markdown, expectedPageId) && isEditableMarkdownPageType(markdown, allowClaim, allowQuestion);
}
export function validateActivityMarkdown(markdown: string, expectedPageId: string): boolean { return validateEditablePageMarkdown(markdown, expectedPageId, true, true); }
