...
export class NoteTagService {
  ...
  async remove(ownerId: string, request: NoteRemoveTagRequest): Promise<NoteRemoveTagResult> {
    const target = this.#targets.resolveTrashTarget(ownerId, {
      activeVaultId: request.activeVaultId, pageId: request.currentPageId,
      renderContextId: request.renderContextId, expectedRevision: request.expectedRevision
    });
    if (target.status !== "ready") return removeClosedResult(request, target.status);
    if (!target.assertCurrent()) return removeClosedResult(request, "stale");
    const opened = this.#editor.open({ activeVaultId: request.activeVaultId, pageId: request.currentPageId });
    ...
    if (!expectedFrontmatter) return removeClosedResult(request, "failed");
    const saved = this.#editor.save({
      requestId: internalRequestId(request.requestId, "remove"), activeVaultId: request.activeVaultId,
      pageId: request.currentPageId, expectedRevisionId: opened.revisionId,
      renderIdentity: opened.renderIdentity, markdown
    });
    if (saved.status !== "committed") return removeClosedResult(request, mapSaveStatus(saved.status));
    let render: NoteRenderResult;
...
function internalRequestId(requestId: string, action: "add" | "remove" = "add"): string {
  const suffix = createHash("sha256").update(`pige.note-tag-${action}.v1\0${requestId}`, "utf8").digest("hex").slice(0, 32);
...
function removeTagFromMarkdown(markdown: string, requestedTag: string, updatedAt: string): string | undefined {
  ...
  const nextTags = tags.filter((tag) => createPigeTagKey(tag) !== tagKey);
  const nextRaw = parsed.raw.replace(/^tags:[^\r\n]*$/mu, `tags: ${JSON.stringify(nextTags)}`)
    .replace(/^updated_at:[^\r\n]*$/mu, `updated_at: ${updatedAt}`);
  return `${markdown.slice(0, rawStart)}${nextRaw}${markdown.slice(rawStart + parsed.raw.length)}`;
}
...
function removeClosedResult(request: NoteRemoveTagRequest, status: Exclude<NoteRemoveTagResult["status"], "committed">): NoteRemoveTagResult {
  return { ...request, status };
}
