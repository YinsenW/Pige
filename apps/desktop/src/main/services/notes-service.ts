...
export type NotesTrashResolution =
  | {
      ...
      readonly pageContentHash: string; readonly pageType?: string;
...
export class NotesService {
  ...
  async render(request: NoteRenderRequest, ownerId?: string): Promise<NoteRenderResult> {
    ...
    const questionState = parsedFrontmatter && stable.document.summary.pageType === "question" ? readQuestionState(parsedFrontmatter.raw) : undefined, claimConfidence = parsedFrontmatter && stable.document.summary.pageType === "claim" ? readClaimConfidence(parsedFrontmatter.raw) : undefined, entityType = parsedFrontmatter && stable.document.summary.pageType === "entity" ? readEntityType(parsedFrontmatter.raw) : undefined, entityMentions = parsedFrontmatter && stable.document.summary.pageType === "entity" ? projectEntityMentions(vaultPath, stable.document.summary.pageId) : undefined, questionAnswers = parsedFrontmatter && stable.document.summary.pageType === "question" ? projectQuestionAnswers(vaultPath, parsedFrontmatter.raw) : undefined, claimContradictions = parsedFrontmatter && stable.document.summary.pageType === "claim" ? projectClaimContradictions(vaultPath, parsedFrontmatter.raw) : undefined, claimSupports = parsedFrontmatter && stable.document.summary.pageType === "claim" ? projectClaimSupports(vaultPath, parsedFrontmatter.raw) : undefined, claimSupersessions = parsedFrontmatter && stable.document.summary.pageType === "claim" ? projectClaimSupersessions(vaultPath, parsedFrontmatter.raw) : undefined, claimEvidence = parsedFrontmatter && stable.document.summary.pageType === "claim" ? projectClaimEvidence(vaultPath, parsedFrontmatter.raw) : undefined, conceptParents = parsedFrontmatter && stable.document.summary.pageType === "concept" ? projectConceptParents(vaultPath, parsedFrontmatter.raw) : undefined, topicParents = parsedFrontmatter && stable.document.summary.pageType === "topic" ? projectTopicParents(vaultPath, parsedFrontmatter.raw) : undefined, referenceIndexRevision = this.#referenceIndex?.inlineReferenceRevision(vaultPath);
    ...
    return {
      ...
      ...(renderContextId ? {
        ...
        ...(stable.document.summary.pageType !== "note" && isLifecycleKnowledgePage(stable.document.summary.pageType, stable.document.summary.status) ? { archiveEligibility: { canArchive: stable.document.summary.status === "active", revision: publicEditorRevision(stable.pageContentHash) }, restoreEligibility: { canRestore: stable.document.summary.status === "archived", revision: publicEditorRevision(stable.pageContentHash) } } : {}),
        ...
        ...(claimConfidence ? { claimConfidence: { confidence: claimConfidence, canChange: stable.document.summary.status === "active", revision: publicEditorRevision(stable.pageContentHash) } } : {}), ...(entityType ? { entityType: { entityType, canChange: stable.document.summary.status === "active", revision: publicEditorRevision(stable.pageContentHash) } } : {}), ...(entityMentions ? { entityMentions: { items: [...entityMentions], canEdit: stable.document.summary.status === "active", revision: publicEditorRevision(stable.pageContentHash) } } : {}),
        ...
        ...(claimContradictions ? { claimContradictions: { items: [...claimContradictions], canEdit: stable.document.summary.status === "active", revision: publicEditorRevision(stable.pageContentHash) } } : {}), ...(claimSupports ? { claimSupports: { items: [...claimSupports], canEdit: stable.document.summary.status === "active", revision: publicEditorRevision(stable.pageContentHash) } } : {}), ...(claimSupersessions ? { claimSupersessions: { items: [...claimSupersessions], canEdit: stable.document.summary.status === "active", revision: publicEditorRevision(stable.pageContentHash) } } : {}), ...(claimEvidence ? { claimEvidence: { items: [...claimEvidence], canEdit: stable.document.summary.status === "active", revision: publicEditorRevision(stable.pageContentHash) } } : {}), ...(conceptParents ? { conceptParents: { items: [...conceptParents], canEdit: stable.document.summary.status === "active", revision: publicEditorRevision(stable.pageContentHash) } } : {}), ...(topicParents ? { topicParents: { items: [...topicParents], canEdit: stable.document.summary.status === "active", revision: publicEditorRevision(stable.pageContentHash) } } : {})
      } : {})
    };
  }
  ...
  resolveTrashTarget(ownerId: string, input: NotesManagedPageTargetInput): NotesTrashResolution { return this.resolveManagedPageTarget(ownerId, input, "trashable_page"); } resolveLifecycleTarget(ownerId: string, input: NotesManagedPageTargetInput): NotesTrashResolution { return this.resolveManagedPageTarget(ownerId, input, "lifecycle_page"); }
  resolveManagedPageTarget(ownerId: string, input: NotesManagedPageTargetInput, pageType: "note" | "question" | "claim" | "concept" | "entity" | "topic" | "trashable_page" | "lifecycle_page"): NotesTrashResolution {
    const vault = this.#vaults.current();
    const vaultPath = this.#vaults.activeVaultPath();
    ...
    const parsed = parsePigeFrontmatter(context.markdown), trashable = isTrashableKnowledgePage(context.pageType, parsed?.frontmatter.status), lifecycle = isLifecycleKnowledgePage(context.pageType, parsed?.frontmatter.status);
    if (pageType === "trashable_page" ? !trashable : pageType === "lifecycle_page" ? !lifecycle : context.pageType !== pageType) return { status: "ineligible" };
    ...
    return {
      ...
      pageContentHash: context.pageContentHash, pageType: context.pageType,
