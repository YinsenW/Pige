...
export class NotesService {
  ...
  async render(request: NoteRenderRequest, ownerId?: string): Promise<NoteRenderResult> {
    ...
    return {
      ...
      ...(renderContextId ? {
        ...
        ...(claimConfidence ? { claimConfidence: { confidence: claimConfidence, canChange: stable.document.summary.status === "active", revision: publicEditorRevision(stable.pageContentHash) } } : {}),
  ...
  resolveManagedPageTarget(ownerId: string, input: NotesManagedPageTargetInput, pageType: "note" | "question" | "claim" | "concept" | "trashable_page" | "lifecycle_page"): NotesTrashResolution {
    const vault = this.#vaults.current();
    const vaultPath = this.#vaults.activeVaultPath();
