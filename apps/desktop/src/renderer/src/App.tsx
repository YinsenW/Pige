...
import { HomeJobAction } from "./components/HomeJobAction"; import { createDependencyRepairAction, createDependencyRepairHandler } from "./components/home-dependency-repair";
...
export function App(): React.JSX.Element {
  ...
  const loadNoteMergeTargets = (currentPageId: string, pageType: "note" | "entity"): Promise<readonly ReaderNoteMergeTarget[]> => loadKnowledgePageTargets(currentPageId, [pageType]);
  ...
  const repairDependencyJob = createDependencyRepairHandler({ activeVaultId: () => activeVaultIdRef.current, repair: (request) => window.pige.jobs.repairDependency(request), onRepaired: async (job) => { setCaptureToast({ kind: "success", message: t("home.jobRequeued"), queuedJobId: job.id }); await refreshVaultState(); } });
      ...
      >
        ...
        {blocked ? (
          ...
          <HomeComposer
            ...
            onCancelJob={cancelJob} onRetryJob={retryJob} onRepairDependency={repairDependencyJob}
...
export function LibraryPanel(props: {
  ...
  readonly onLoadNoteMergeTargets: (currentPageId: string, pageType: "note" | "entity") => Promise<readonly ReaderNoteMergeTarget[]>;
  ...
  const mergeSelectedNote = async (target: ReaderNoteMergeTarget): Promise<ReaderNoteMergeOutcome> => {
    ...
    const activeVaultId = props.activeVaultId;
    const renderContextId = note?.renderContextId;
    if (!note || !isMergeEligible(note) || !revision || !activeVaultId || !renderContextId) {
    ...
    try {
      ...
      if (!noteMergeIdentityMatches(request, result) || result.status !== "committed" ||
        result.render.summary.pageId !== request.currentPageId || result.render.summary.pageType !== note.summary.pageType) {
        return { status: "retained" };
      }
    ...
    return (
      <section className="library-page reader-page" aria-label={props.t("note.reader")}>
        <header className="reader-toolbar">
          ...
          <div className="reader-toolbar-actions">
            ...
            <ReaderDocumentActions
              ...
              canMerge={isMergeEligible(props.selectedNote) && Boolean(props.activeVaultId && props.selectedNote.renderContextId && props.selectedNote.trashEligibility?.revision)}
              ...
              mergeLabels={readerNoteMergeLabels(props.t, summary.pageType)}
              ...
              onLoadMergeTargets={() => props.onLoadNoteMergeTargets(summary.pageId, summary.pageType === "entity" ? "entity" : "note")}
              onMerge={mergeSelectedNote}
              onLoadRelateTargets={() => props.onLoadNoteRelateTargets
                ? props.onLoadNoteRelateTargets(summary.pageId)
                : props.onLoadNoteMergeTargets(summary.pageId, "note")}
        ...
        <NoteReader
          ...
          onSetQuestionState={(request) => window.pige.notes.setQuestionState(request)} onSetClaimConfidence={(request) => window.pige.notes.setClaimConfidence(request)} onSetEntityType={(request) => window.pige.notes.setEntityType(request)} onReadEntityIdentifiers={(request) => window.pige.notes.readEntityIdentifiers(request)} onChangeEntityIdentifier={(request) => window.pige.notes.changeEntityIdentifier(request)} onSearchEntityMentions={(request) => window.pige.notes.searchEntityMentions(request)} onChangeEntityMention={(request) => window.pige.notes.changeEntityMention(request)} onSearchQuestionAnswers={(request) => window.pige.notes.searchQuestionAnswers(request)} onChangeQuestionAnswer={(request) => window.pige.notes.changeQuestionAnswer(request)} onSearchClaimContradictions={(request) => window.pige.notes.searchClaimContradictions(request)} onChangeClaimContradiction={(request) => window.pige.notes.changeClaimContradiction(request)} onSearchClaimSupports={(request) => window.pige.notes.searchClaimSupports(request)} onChangeClaimSupport={(request) => window.pige.notes.changeClaimSupport(request)} onSearchClaimSupersessions={(request) => window.pige.notes.searchClaimSupersessions(request)} onChangeClaimSupersession={(request) => window.pige.notes.changeClaimSupersession(request)} onSearchClaimEvidence={(request) => window.pige.notes.searchClaimEvidence(request)} onChangeClaimEvidence={(request) => window.pige.notes.changeClaimEvidence(request)} onSearchConceptParents={(request) => window.pige.notes.searchConceptParents(request)} onChangeConceptParent={(request) => window.pige.notes.changeConceptParent(request)} onSearchTopicParents={(request) => window.pige.notes.searchTopicParents(request)} onChangeTopicParent={(request) => window.pige.notes.changeTopicParent(request)}
...
function HomeComposer(props: {
  ...
  readonly onRetryJob: (jobId: string) => Promise<unknown>; readonly onRepairDependency: (job: JobSummary) => Promise<unknown>;
  ...
  const mergeSelectedHomeNote = async (target: ReaderNoteMergeTarget): Promise<ReaderNoteMergeOutcome> => {
    ...
    const renderContextId = note?.renderContextId;
    const revision = note?.trashEligibility?.revision;
    if (!note || !isMergeEligible(note) || !activeVaultId || !renderContextId || !revision) return { status: "retained" };
    const request: NoteMergeRequest = {
      apiVersion: 1,
      requestId: createNoteMergeRequestId(),
      activeVaultId,
    ...
    try {
      ...
      if (
        ...
        result.render.summary.pageType !== note.summary.pageType
      ) return { status: "retained" };
      return { status: "committed", render: result.render };
    } catch {
      return { status: "retained" };
        ...
        >
          <header className="task-header">
            ...
            {!processingListExpanded && visibleRecentJobs[0] ? (() => {
              ...
              const repair = sourceRepairAction(currentJob) ?? createDependencyRepairAction(currentJob, props.t("home.retryJob"), props.t("home.jobWaiting"), () => props.onRepairDependency(currentJob));
          ...
          {processingListExpanded ? (
            ...
            {visibleRecentJobs.map((job) => {
              ...
              const repair = sourceRepairAction(job) ?? createDependencyRepairAction(job, props.t("home.retryJob"), props.t("home.jobWaiting"), () => props.onRepairDependency(job));
            ...
            <>
              <div className="settings-inline-actions">
                ...
                <ReaderDocumentActions
                  ...
                  canMerge={isMergeEligible(selectedNote) && Boolean(props.activeVault && selectedNote.renderContextId && selectedNote.trashEligibility?.revision)}
                  ...
                  labels={readerDocumentActionLabels(props.t)}
                  mergeLabels={readerNoteMergeLabels(props.t, selectedNote.summary.pageType)}
                  relateLabels={readerNoteRelateLabels(props.t)}
                  ...
                  onLoadMergeTargets={() => props.onLoadNoteMergeTargets(selectedNote.summary.pageId,
                    selectedNote.summary.pageType === "entity" ? "entity" : "note")}
...
function readerNoteMergeLabels(t: (key: string) => string, pageType: NoteRenderResult["summary"]["pageType"]) {
  const prefix = pageType === "entity" ? "entity.merge" : "note.merge";
  return { title: t(`${prefix}.title`), description: t(`${prefix}.description`),
    survivor: t(`${prefix}.survivor`), target: t(`${prefix}.target`), loading: t(`${prefix}.loading`),
    empty: t(`${prefix}.empty`), cancel: t(`${prefix}.cancel`), confirm: t(`${prefix}.confirm`),
    pending: t(`${prefix}.pending`), failed: t(`${prefix}.failed`) };
}

function isMergeEligible(note: NoteRenderResult): boolean { return note.summary.status === "active" &&
  (note.summary.pageType === "note" || note.summary.pageType === "entity"); }
...
function noteMarkdownEditorLabels(t: (key: string) => string): NoteMarkdownEditorLabels {
  return {
    ...
    discardTitle: t("note.editor.discard.title"),
