      ...
      >
        ...
        {blocked ? (
          ...
          <LibraryPanel
            ...
            onCurrentNoteSourceReconnected={adoptReconnectedNote} onArchiveCurrentNote={(request) => window.pige.notes.archiveCurrent(request)} onCurrentNoteArchived={adoptMergedNote} onRestoreArchivedNote={(request) => window.pige.notes.restoreArchived(request)} onCurrentNoteRestored={adoptMergedNote} onRenameCurrentNote={(request) => window.pige.notes.rename(request)} onCurrentNoteRenamed={adoptMergedNote} onAddNoteTag={(request) => window.pige.notes.editTaxonomy(request)} onCurrentNoteTagged={adoptMergedNote}
        ...
        <header className="reader-toolbar">
          ...
          <div className="reader-toolbar-actions">
            ...
            <ReaderDocumentActions
              ownerIdentity={`${props.activeVaultId ?? ""}:${summary.pageId}:${props.selectedNote.renderContextId ?? ""}:${props.selectedNote.trashEligibility?.revision ?? ""}:${props.selectedNote.archiveEligibility?.revision ?? ""}:${props.selectedNote.restoreEligibility?.revision ?? ""}:${props.selectedNote.renameEligibility?.revision ?? ""}:${props.selectedNote.tagging?.revision ?? ""}`}
...
function HomeComposer(props: {
  ...
  const archiveSelectedHomeNote = () => submitReaderNoteArchive({ note: selectedNoteRef.current, activeVaultId: activeVaultIdRef.current, submit: (request) => window.pige.notes.archiveCurrent(request), currentNote: () => selectedNoteRef.current }); const restoreSelectedHomeNote = () => submitReaderNoteRestore({ note: selectedNoteRef.current, activeVaultId: activeVaultIdRef.current, submit: (request) => window.pige.notes.restoreArchived(request), currentNote: () => selectedNoteRef.current }); const renameSelectedHomeNote = (title: string) => submitReaderNoteRename({ note: selectedNoteRef.current, activeVaultId: activeVaultIdRef.current, title, submit: (request) => window.pige.notes.rename(request), currentNote: () => selectedNoteRef.current }); const addTagToSelectedHomeNote = (tags: readonly string[], topics: readonly string[]) => submitReaderNoteTag({ note: selectedNoteRef.current, activeVaultId: activeVaultIdRef.current, tags, topics, submit: (request) => window.pige.notes.editTaxonomy(request), currentNote: () => selectedNoteRef.current });
          ...
          {editorReady && isNoteEditorEligible(selectedNote) ? (
            ...
            <>
              <div className="settings-inline-actions">
                ...
                />
                <ReaderDocumentActions
                  ownerIdentity={`${props.activeVault?.vaultId ?? ""}:${selectedNote.summary.pageId}:${selectedNote.renderContextId ?? ""}:${selectedNote.trashEligibility?.revision ?? ""}:${selectedNote.archiveEligibility?.revision ?? ""}:${selectedNote.restoreEligibility?.revision ?? ""}:${selectedNote.renameEligibility?.revision ?? ""}:${selectedNote.tagging?.revision ?? ""}`}
                  ...
                  canArchive={selectedNote.archiveEligibility?.canArchive === true && Boolean(props.activeVault && selectedNote.renderContextId)} archiveLabels={readerDocumentArchiveLabels(props.t)} canRestore={selectedNote.restoreEligibility?.canRestore === true && Boolean(props.activeVault && selectedNote.renderContextId)} restoreLabels={readerDocumentRestoreLabels(props.t)} canRename={selectedNote.renameEligibility?.canRename === true && Boolean(props.activeVault && selectedNote.renderContextId)} renameLabels={readerNoteRenameLabels(props.t)} canAddTag={selectedNote.summary.status === "active" && selectedNote.tagging?.canEdit === true && Boolean(props.activeVault && selectedNote.renderContextId)} existingTags={selectedNote.tagging?.tags ?? []} existingTopics={selectedNote.tagging?.topics ?? []} tagLabels={readerNoteTagLabels(props.t)}
                  currentTitle={selectedNote.summary.title}
                  labels={readerDocumentActionLabels(props.t)}
                  mergeLabels={readerNoteMergeLabels(props.t)}
                  relateLabels={readerNoteRelateLabels(props.t)}
                  onMoveToTrash={trashSelectedHomeNote}
                  onArchive={archiveSelectedHomeNote} onArchiveCommitted={adoptMergedHomeNote} onRestore={restoreSelectedHomeNote} onRestoreCommitted={adoptMergedHomeNote} onRename={renameSelectedHomeNote} onRenameCommitted={adoptMergedHomeNote} onAddTag={addTagToSelectedHomeNote} onTagCommitted={adoptMergedHomeNote}
