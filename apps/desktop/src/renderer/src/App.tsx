...
function HomeComposer(props: {
  ...
  const archiveSelectedHomeNote = () => submitReaderNoteArchive({ note: selectedNoteRef.current, activeVaultId: activeVaultIdRef.current, submit: (request) => window.pige.notes.archiveCurrent(request), currentNote: () => selectedNoteRef.current }); const restoreSelectedHomeNote = () => submitReaderNoteRestore({ note: selectedNoteRef.current, activeVaultId: activeVaultIdRef.current, submit: (request) => window.pige.notes.restoreArchived(request), currentNote: () => selectedNoteRef.current }); const renameSelectedHomeNote = (title: string) => submitReaderNoteRename({ note: selectedNoteRef.current, activeVaultId: activeVaultIdRef.current, title, submit: (request) => window.pige.notes.rename(request), currentNote: () => selectedNoteRef.current }); const changeSelectedHomeNoteAlias = (action: "add" | "remove", alias: string) => submitReaderNoteAliasChange({ note: selectedNoteRef.current, activeVaultId: activeVaultIdRef.current, action, alias, submit: (request) => window.pige.notes.changeAlias(request), currentNote: () => selectedNoteRef.current }); const addTagToSelectedHomeNote = (tags: readonly string[], topics: readonly string[]) => submitReaderNoteTag({ note: selectedNoteRef.current, activeVaultId: activeVaultIdRef.current, tags, topics, submit: (request) => window.pige.notes.editTaxonomy(request), currentNote: () => selectedNoteRef.current });
