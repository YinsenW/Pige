...
function HomeComposer(props: {
  ...
  const archiveSelectedHomeNote = () => submitReaderNoteArchive({ note: selectedNoteRef.current, activeVaultId: activeVaultIdRef.current, submit: (request) => window.pige.notes.archiveCurrent(request), currentNote: () => selectedNoteRef.current }); const restoreSelectedHomeNote = () => submitReaderNoteRestore({ note: selectedNoteRef.current, activeVaultId: activeVaultIdRef.current, submit: (request) => window.pige.notes.restoreArchived(request), currentNote: () => selectedNoteRef.current }); const renameSelectedHomeNote = (title: string) => submitReaderNoteRename({ note: selectedNoteRef.current, activeVaultId: activeVaultIdRef.current, title, submit: (request) => window.pige.notes.rename(request), currentNote: () => selectedNoteRef.current }); const changeSelectedHomeNoteAlias = (action: "add" | "remove", alias: string) => submitReaderNoteAliasChange({ note: selectedNoteRef.current, activeVaultId: activeVaultIdRef.current, action, alias, submit: (request) => window.pige.notes.changeAlias(request), currentNote: () => selectedNoteRef.current }); const addTagToSelectedHomeNote = (tag: string) => submitReaderNoteTag({ note: selectedNoteRef.current, activeVaultId: activeVaultIdRef.current, tag, submit: (request) => window.pige.notes.addTag(request), currentNote: () => selectedNoteRef.current });
            ...
            <>
              <div className="settings-inline-actions">
                ...
                <ReaderDocumentActions
                  ...
                  canArchive={selectedNote.archiveEligibility?.canArchive === true && Boolean(props.activeVault && selectedNote.renderContextId)} archiveLabels={readerDocumentArchiveLabels(props.t)} canRestore={selectedNote.restoreEligibility?.canRestore === true && Boolean(props.activeVault && selectedNote.renderContextId)} restoreLabels={readerDocumentRestoreLabels(props.t)} canRename={selectedNote.renameEligibility?.canRename === true && Boolean(props.activeVault && selectedNote.renderContextId)} renameLabels={readerNoteRenameLabels(props.t)} canManageAliases={Boolean(props.activeVault && selectedNote.renderContextId && (selectedNote.aliasing?.canAdd || selectedNote.aliasing?.canRemove))} aliases={selectedNote.aliasing?.aliases ?? []} aliasLabels={readerNoteAliasLabels(props.t)} canAddTag={selectedNote.summary.status === "active" && selectedNote.tagging?.canAdd === true && Boolean(props.activeVault && selectedNote.renderContextId)} existingTags={selectedNote.tagging?.tags ?? []} tagLabels={readerNoteTagLabels(props.t)}
                  currentTitle={selectedNote.summary.title}
                  labels={readerDocumentActionLabels(props.t)}
                  mergeLabels={readerNoteMergeLabels(props.t)}
                  relateLabels={readerNoteRelateLabels(props.t)}
                  onMoveToTrash={trashSelectedHomeNote}
                  onArchive={archiveSelectedHomeNote} onArchiveCommitted={adoptMergedHomeNote} onRestore={restoreSelectedHomeNote} onRestoreCommitted={adoptMergedHomeNote} onRename={renameSelectedHomeNote} onRenameCommitted={adoptMergedHomeNote} onAliasChange={changeSelectedHomeNoteAlias} onAliasCommitted={adoptMergedHomeNote} onAddTag={addTagToSelectedHomeNote} onTagCommitted={adoptMergedHomeNote}
