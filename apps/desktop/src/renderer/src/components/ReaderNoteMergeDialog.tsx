...
export interface ReaderNoteMergeTarget {
  readonly pageId: string;
  readonly title: string;
  readonly updatedAt: string;
}

export type ReaderNoteMergeOutcome =
  | { readonly status: "committed"; readonly render: NoteRenderResult }
  | { readonly status: "retained" };

export interface ReaderNoteMergeLabels {
  readonly title: string;
...
export function ReaderNoteMergeDialog(props: {
  readonly ownerIdentity: string;
  readonly currentTitle: string;
  readonly returnFocusRef: RefObject<HTMLButtonElement | null>;
  readonly labels: ReaderNoteMergeLabels;
  ...
  const submit = async (): Promise<void> => {
    ...
    setState("pending");
    try {
      const outcome = await props.onMerge(target);
      if (sequence !== sequenceRef.current || ownerIdentity !== ownerIdentityRef.current) return;
  ...
  return (
    <div className="confirmation-backdrop">
      <section
        ...
        role="alertdialog"
        aria-modal="true"
        aria-labelledby="reader-note-merge-title"
        aria-describedby="reader-note-merge-description"
      ...
      >
        ...
        <div className="confirmation-copy">
          <h2 id="reader-note-merge-title">{props.labels.title}</h2>
          <p id="reader-note-merge-description">{props.labels.description}</p>
          <p><strong>{props.labels.survivor}</strong> {props.currentTitle}</p>
