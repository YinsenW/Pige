...
- Preserved PDF/DOCX/PPTX/image sources create metadata-only source pages and a `parse` or `ocr` job. Document jobs are queued when a bundled adapter resolves. Direct images queue on supported macOS 26 systems when the verified local Vision helper resolves and otherwise remain `waiting_dependency`; document OCR remains waiting for a pixel materializer. No source may create `agent_ingest` until a parser/OCR adapter writes and validates an `extracted_text` or `ocr` Artifact.
