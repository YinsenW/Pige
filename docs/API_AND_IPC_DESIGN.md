...
`activity.list` returns bounded pages in deterministic `createdAt` descending then `operationId` ascending order. An optional bounded summary query and `applied|undone` status filter search the full durable Operation projection rather than only the loaded renderer page. Its opaque cursor is process-local and bound to the active Vault, canonical Vault path, full snapshot hash, canonical query/status filter, page offset, and prior-page boundary; invented, restarted, cross-filter, cross-Vault, or changed-snapshot cursors fail stale without exposing paths or Operation bodies.
...
- `collections.listRevisionHistory` binds the active Vault, Dataset and current revision and returns
  bounded safe summaries behind an opaque current-chain cursor. `collections.openRevisionHistory`
  re-proves one reachable immutable revision and returns a mutation-disabled snapshot.
  `collections.restoreRevisionHistory` requires explicit confirmation, re-proves the same current
  chain and publishes the selected schema/payload as one new forward revision with Activity/Undo;
  historical revisions and Source evidence are never rewritten.
