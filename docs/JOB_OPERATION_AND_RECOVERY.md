...
`apply_proposed` binds the same revision, preserves those current bytes as the exact reversible
before-image, applies the immutable proposal, and settles the original Job with its Operation;
further drift returns a refreshed conflict instead of acknowledging or overwriting different bytes.
...
| Dataset revision | create dataset revision with manifest/schema/payload/source hashes; trash/restore/purge Dataset Bundle |
...
Reader save, complete taxonomy correction and confirmed single-tag removal write reversible `update_page`; merge binds one two-page receipt. Revision restore uses an integrity-checked private image and deterministic `restore_page`; retry/restart adopts once and Undo restores the displaced note. Exact note-edit Redo requires the matching Undo and unchanged before image. Agent-created pages and Agent page updates now follow that same matching-Undo rule: create Redo restores the exact private-trash image only into the still-vacant owned path, while update Redo re-proves the exact before image and publishes the receipt-bound after image. Each writes one deterministic forward `restore_page` or `update_page` Operation, adopts an interrupted publication once after restart, and preserves live bytes on path, content, image, relationship, or Operation drift. Active note/claim/question/concept/entity title rename, User Library tag lifecycle, Topic-title rename, exact note merge, Knowledge Health duplicate-topic merge, saved Collection view lifecycle, and Collection cell/row/column/formula/relation/lookup/rollup mutations use the same matching-Undo rule while re-proving every receipt-bound target and immutable revision; each Redo writes one deterministic forward Operation, restart adopts an interrupted commit, repeated Undo/Redo keeps exact images or immutable revisions, and drift preserves live state. Memory commits private receipts before `create_memory`/`update_memory`/`trash_memory`; matching Undo/Redo re-proves the receipt and current registry, preserves unrelated atoms, publishes one deterministic forward Operation per cycle, and restart adopts an interrupted cycle once. Conversation trash/restore binds JSONL hash/revision/receipt without Provider replay. Conversation export rechecks the tail before external write and creates no Job/Operation. Knowledge Health repair uses reversible `update_page` bound to report/index and exact source/target, orphan/parent, or duplicate-topic survivor/absorbed proofs. Memory backup inclusion changes only outside active Backup work, commits the revision-fenced vault config before one pathless `change_setting` Operation, and rolls config back if that Operation cannot be published. The preference affects new Backup Jobs only. Default source-storage changes bind the active Vault and exact source-storage revision, persist a private receipt before changing `.pige/config.json`, and publish one pathless `change_setting` Operation. Activity Undo and startup recovery converge the exact prior or next strategy; both directions affect future file captures only and never move, rewrite, or delete existing source bytes.
...
- Source evidence uses its own `trash_source_asset` / `restore_source_asset` owner: one private receipt
  ...
  and active-Job use are disproved. Restore, matching Activity Redo, Activity Undo and restart re-prove
  that receipt and adopt one deterministic forward Operation, while uncertain references, occupied
  destinations or identity drift fail closed.
...
- Confirmed managed Dataset purge re-proves the exact trash receipt, immutable Bundle tree and
  `trash_dataset` Operation, then persists a tombstone and irreversible `purge_dataset` Operation
  before payload removal. Restart adopts or finishes only that quarantined Bundle; source evidence
  remains outside the deleted tree.
...
- Activity History pages or searches the authoritative Operation projection by bounded
  safe summary and applied/undone state. Process-local cursors bind Vault, snapshot,
  canonical filters and boundary; stale paging or search preserves current rows and does
  not change existing forward-writing Undo/Redo recovery.
