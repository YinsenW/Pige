...
- Document parse jobs transition through `queued` -> `running` -> `completed`/`completed_with_warnings`, preserve deterministic checksummed text/metadata artifacts, and create OCR jobs for sparse or image-bearing content. If local OCR is ready, mixed-PDF Agent ingest waits for enrichment; if OCR is unavailable at parse time or disappears before the OCR Job runs, medium/high verified native text may proceed with an OCR-pending review warning while the OCR Job stays `waiting_dependency`. Low/no coverage still waits for readable OCR.
...
- Startup and vault activation first reconcile interrupted jobs. Running capture/parse/OCR/Agent-ingest/index jobs already proven idempotent are requeued; cancellation-in-progress and unproven classes become `failed_retryable` with an explanation. Waiting Agent-ingest Jobs are also reconsidered after current model/OCR capability probes, but remain waiting while selected PDF OCR is runnable. The main-process scheduler then drains queued capture, document parse, OCR, and Agent ingest work in bounded batches of 20, yields between batches, and coalesces concurrent schedule requests so a drop larger than one batch does not stall after the first 20 sources.
