...
- Queued DOCX/PPTX captures create metadata-only pages then parse Jobs. Preserved PDFs create metadata-only pages and Agent-ingest Jobs; Pi may persist a deterministic parse child. Images queue OCR when the verified helper is ready and otherwise wait for that dependency.
...
- Phase 3 text/PDF pages create deterministic `agent_ingest`. Missing models wait; setup requeues without duplicating sources. A PDF parse child keys parent/tool/version/source revision/input, reuses one semantic action across Pi call IDs, and never stores raw IDs.
