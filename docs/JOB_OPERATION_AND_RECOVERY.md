...
- `waiting_dependency` pauses for a missing model, tool, capability, binding, or source.
  Backup uses it body-free for missing/rebound roots and resumes the same Job after repair.
...
- `BackupCoordinatorService` retains five transitions pending generic coordinator migration.
