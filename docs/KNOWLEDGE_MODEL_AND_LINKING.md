...
The bounded read-only slice rebuilds durable Markdown through Local Database Service into body-free domain/topic/concept/source aggregates and supporting-page links; missing indexes degrade typed-empty and renderer never reads vault. Evidence density is `fragmentPageCount + sourceCount` in `0`, `1–2`, `3–5`, `6+`; domain/topic size uses weight, deeper size uses relative density, exact leaf count never depth, and `needs_review` outlines without replacing text/a11y detail. Branch collapse is renderer-local: standard Left/Right tree keys collapse, expand, or traverse; searching a hidden match reveals only its exact ancestors; Show all clears the local collapse set and never writes graph state. Explicit Topic- and Concept-parent edits feed the same rebuildable tree and backlink index. Health, force graph, incremental/10k, and packaged parity remain open.
