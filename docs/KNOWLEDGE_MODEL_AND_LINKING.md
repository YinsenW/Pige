...
- The explicit durable direction is the current Topic page's stable parent Topic IDs in `topics`;
  child edges are rebuilt and never separately mutated.
- Reader add/remove binds the active Vault, current Topic render/revision, and exact current parent.
  Main rejects cycles or malformed hierarchy truth before mutation, applies one atomic `update_page`,
  and Activity Undo/Redo plus restart restore the exact prior/next hierarchy bytes.
...
- Active note, claim, question, concept and entity title changes preserve stable page identity and
  page type, retain the prior title as an alias and use exact reversible `rename_page`; Topic title
  changes remain with the reference-aware Topic owner.
...
The bounded read-only slice rebuilds durable Markdown through Local Database Service into body-free domain/topic/concept/source aggregates and supporting-page links; missing indexes degrade typed-empty and renderer never reads vault. Evidence density is `fragmentPageCount + sourceCount` in `0`, `1–2`, `3–5`, `6+`; domain/topic size uses weight, deeper size uses relative density, exact leaf count never depth, and `needs_review` outlines without replacing text/a11y detail. Branch collapse is renderer-local: standard Left/Right tree keys collapse, expand, or traverse. Local title/type search returns at most 20 deterministic body-free matches with ancestor breadcrumbs, temporarily reveals every hit path, and opens only its stable page ID; clearing restores the prior collapse state, while tree-owner drift clears stale results without navigation. Show all clears the local collapse set and never writes graph state. Explicit Topic- and Concept-parent edits feed the same rebuildable tree and backlink index. Health, force graph, incremental/10k, and packaged parity remain open.
...
changes, and records reversible `update_page`. Ambiguous/complex and other repairs stay open.
