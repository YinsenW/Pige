...
- The Vault display name is portable manifest metadata. Renaming it never changes the
  stable Vault ID or filesystem path and commits only against the exact current metadata
  revision.
...
- Note reader with source metadata and related pages; Reader/Library can correct active-note
  tags/topics, explicitly confirm removal of one existing tag, rename the note, and manage bounded
  aliases without changing stable identity. Closed mutations retain the current Reader/draft, while
  successful changes remain undoable.
...
- Knowledge Base settings must include a real Vault & Note Storage page for local note storage: current vault name with a path-independent rename action, active vault path, knowledge root path, managed source-copy path (the v1 UI compatibility label is Source asset root), default source storage strategy, reveal in file manager, exact reconnect for unavailable referenced originals, explicit safe relocation that preserves the original Vault, open existing vault folder, create new vault, recent vaults, backup, restore, trash policy, and safe index repair entry points. Reconnect accepts only the same content and format against current Main-owned proof, preserves the Source ID, and leaves the current record unchanged on cancel, mismatch, or stale state.
