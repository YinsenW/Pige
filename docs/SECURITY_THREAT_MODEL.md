...
- The semantic PDF converter receives only bytes from one verified preserved snapshot, has no path, hosted API, CLI, runtime-download, or network authority, and cannot write vault artifacts. A separate PDF.js bridge reads the same bounded snapshot for page metadata, native-text locators, and OCR candidates; validated results return to the main-process Parser Service for atomic writes.
...
- Parser startup depends only on exact bundled `@firecrawl/anydoc`, `pdfjs-dist`, `@napi-rs/canvas`, yauzl, and fast-xml-parser packages recorded in the dependency/toolchain manifests. Ordinary ingest never installs or repairs packages.
- The current Office worker receives one verified DOCX/PPTX snapshot's bytes and explicit byte/archive/XML/slide/text/media limits, cannot write vault artifacts, and uses exact bundled AnyDoc only for semantic Markdown/Document conversion; it has no Firecrawl API, CLI, or network authority.
