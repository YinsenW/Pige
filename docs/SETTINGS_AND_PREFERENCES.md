...
| Recent vault list and reconnect binding | Vault & Note Storage | `machine_local` | Vault Runtime Service | OS app data | No | `none` for Forget; `os_permission` for the Main directory picker | Exact-revision immediate mutation; active entries are blocked, Forget never deletes Vault bytes, and Reconnect preserves Vault ID after manifest proof |
...
| Parser/toolchain health | Local Capabilities/System | `derived_status` | Runtime Capability Service, Local Tool Service | `resources/toolchain-manifest/` plus resolved bundled paths | No | `none` | Recomputed/repair job; a Ready recheck idempotently requeues preserved waiting parse and Agent-ingest Jobs |
...
- Included copies restore as `root_vault_managed` without external bindings.
