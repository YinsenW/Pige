...
- Derived output binds its sidecar revision. Parsing/direct-image OCR pass the pre-Artifact record as `SourcePageService`'s baseline; PDF/PPTX OCR replaces against the reread checksum, then refreshes that revision. A confirmed PDF Source Refresh records its dynamic parser/render/OCR output tree in one private before/after receipt; failure, Undo, or restart restores that tree with the Source Record. Drift fails rather than mixing revisions.
