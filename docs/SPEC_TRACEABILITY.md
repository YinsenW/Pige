...
| PIGE-VAULT-002 | Stable IDs are independent from slugs and file paths; active-note title/filename rename preserves `page_id`, and Managed Collection table display-name rename preserves `tableId`. | `docs/SYNC_CONFLICT_AND_MIGRATION.md` | P1 | M1 | unit |
| PIGE-VAULT-003 | After Send, each accepted local file gets one private Job snapshot; managed copy adopts it, while reference-original keeps provenance and tools read the snapshot. An unavailable referenced original relinks only after exact proof and preserves its stable Source ID; changed content is previewed before confirmed refresh publication. | `docs/SOURCE_STORAGE_STRATEGY.md` | P2 | M2 | integration |
...
| PIGE-VAULT-005 | Users can rename the portable Vault display label without changing Vault ID/path, configure the knowledge root and an independently configurable managed source-copy location exposed through the v1 Source asset root compatibility/UI name; derived artifacts remain portable under the knowledge root, and Vault & Note Storage controls remain explicit and recoverable. | `docs/DATA_ARCHITECTURE.md` | P1 | M1 | integration |
| PIGE-VAULT-006 | Pige-managed Markdown writes, including recoverable note rename, are atomic and checksum-aware; external changes create a visible conflict instead of being overwritten silently. | `docs/SYNC_CONFLICT_AND_MIGRATION.md` | P1 | M1 | integration |
...
| PIGE-KNOW-001 | Tags are lightweight facets with exact reversible active-note correction/removal; topics, concepts, entities, claims, and questions are durable Markdown pages when they matter. | `docs/KNOWLEDGE_MODEL_AND_LINKING.md` | P4 | M5 | integration |
...
| PIGE-KNOW-003 | Evidence-bound reversible relationship/merge/hierarchy, bounded Entity-identifier correction, and directed Claim support/supersession correction work auto-apply with Operations; destructive loss or unresolved conflict needs intervention. | `docs/KNOWLEDGE_MODEL_AND_LINKING.md` | P7 | M5 | integration |
...
| PIGE-KNOW-005 | Knowledge Health deterministically reports broken links, orphans, duplicate topics, and unsourced claims; an unsourced Claim citation requires explicit current-source selection plus report/index/claim/source reproof before one recoverable Operation/Undo, while exceptions preserve report state and no broad rewrite is silent. | `docs/UI_PROTOTYPE.md` | P9 | M6 | integration |
...
| PIGE-JOB-001 | Expensive, asynchronous, failure-prone, or recoverable work creates one durable Job before it starts; ordinary tool calls do not create parallel approval Jobs. | `docs/JOB_OPERATION_AND_RECOVERY.md` | P2 | M2 | integration |
| PIGE-JOB-002 | Retry/recovery adopts exact effects, ingress snapshots, reviewed-plan steps, a matching completed Pi provider continuation, and an existing `index_rebuild` Job without duplicates; its active-vault/job/updatedAt-fenced cooperative cancel preserves retry/restart ownership, while started or drifted checkpoints fail closed, and durable assistant/review publication clears provider result/continuation checkpoint data from the completed Job. | `docs/JOB_OPERATION_AND_RECOVERY.md` | P2 | M2 | integration |
...
| PIGE-JOB-004 | Missing model, tool, path, or runtime dependencies use visible retryable `waiting_dependency` state rather than silent failure or data loss; exact referenced-original reconnect resumes only matching current waits, and changed content resumes them only after refresh publication. | `docs/JOB_OPERATION_AND_RECOVERY.md` | P2 | M2 | integration |
...
| PIGE-JOB-006 | Reference-based history is restart-safe; Main may compact only bounded model context by immutable hash/reference summary without rewriting JSONL, while bounded vault search projects safe excerpts and reopens an exact matched event without Provider replay or follow-up authority. | `docs/JOB_OPERATION_AND_RECOVERY.md` | P7 | M5 | integration |
| PIGE-JOB-007 | Home and Index & Maintenance expose durable progress/failure/dependency, searchable and status-filtered Activity/Undo, and exceptional needs-attention state without premature completion; Index & Maintenance consumes the existing `index_rebuild` Job through `jobs.list`, `jobs.cancel`, `jobs.onChanged`, and `maintenance.rebuildLocalDatabase`, rendering numeric body-free progress with one-flight, currentness-fenced cancel, and focus-safe accepted/stale/failure retention. | `docs/JOB_OPERATION_AND_RECOVERY.md` | P2 | M2 | integration |
...
| PIGE-DATA-002 | Dataset inspection and natural-language analysis use bounded typed local query plans through the existing Home tool, including declared same-Dataset joins and grouped aggregates of at most four hops, exact immutable revision/schema/row/range/aggregate evidence refs, and bounded scan/results/evidence hashes; a fifth hop or configured hop-limit excess fails closed, and restart reuses persisted result/history without Provider replay; whole payloads, raw bodies, paths, SQL, and database handles never reach the model or renderer. | `docs/CONTEXT_ASSEMBLY_AND_RETRIEVAL_POLICY.md` | P6 | M5 | integration |
| PIGE-DATA-003 | Managed Collections support stable typed fields, editable views, bounded immutable revision history with forward-only restore, user-created read-only snapshot descriptors pinned to one existing checksummed revision/table without copying payload, bounded citation refs and restart adoption/repair; a descriptor may be recoverably trashed/restored by its stable snapshot ID through a private receipt and atomic rename, one Operation/Activity Undo, and exact restart adoption, never purge. Editable relation/lookup/rollup definitions, revalidated read-only navigation from a relation cell to its exact target record, bounded acyclic nested formulas whose numeric values propagate live into dependent formulas, and reversible Agent row/schema/view changes through Activity/Undo remain supported; snapshot create/list/open/citation/trash/restore fail closed for sender, active vault, current revision, table, source Operation, record hash, or path-confinement drift, project neither paths nor bodies, and introduce no Parquet, DuckDB, parser, or dependency; destructive, external-write, authority, and conflict boundaries remain exceptional. | `docs/DATA_ARCHITECTURE.md` | P7 | M5 | integration |
| PIGE-SEC-001 | API keys stay in machine-local app data and are excluded from backups by default. | `docs/SECURITY_THREAT_MODEL.md` | P3 | M3 | integration |
| PIGE-SEC-002 | Closed high risk confirms explicitly: raw Shell per effect; a registered immutable install/config/auth plan once for exact supply chain/steps/destinations/OAuth, with no reusable grant. | `docs/SECURITY_THREAT_MODEL.md` | P8 | M5 | integration |
| PIGE-SEC-003 | Ask, scoped grants, and explicit YOLO reduce prompts without weakening Main classification, audit, attachment/source authority, or hard boundaries. | `docs/SECURITY_THREAT_MODEL.md` | P8 | M5 | integration |
...
| PIGE-SEC-005 | Connected Provider/model plus Send authorizes the exact user-authored and selected bounded context unchanged; stored credentials stay outside payload content, identity drift blocks, and no content policy or second approval intervenes. | `docs/AGENT_RUNTIME_POLICY_CONTEXT.md` | P3 | M3 | integration |
| PIGE-SEC-006 | A machine-local secret-storage adapter keeps values out of the vault and default backups; Electron OS protection is used when available, with explicit safe fallback state and fail-closed protected decrypt. | `docs/SECURITY_THREAT_MODEL.md` | P1 | M1 | integration |
...
| PIGE-PI-001 | After preservation, upstream Pi is sole semantic orchestrator and its final assistant message is answer authority; Host owns typed transport, tools, high-risk authority, durable reliability, safe presentation, and mutation commits without semantic answer validation or repair loops. Exact Job/event/input/immutable context snapshot/catalog/Provider/model checkpoints adopt only a matching completed continuation, never replay a started or drifted call, and clear provider result/continuation checkpoint data once durable assistant/review publication succeeds. Same-page cited update plus bounded tags is one reversible effect; an exact authored command may drive at most two distinct existing-page effects, independently checkpointed and restart-adopted. | `docs/PI_AGENT_AND_MODEL_PROVIDER_INTEGRATION.md` | P3 | M3 | integration |
...
| PIGE-PI-003 | Pi catalogs derive ambient tools only from explicit authored intent; attachments/fallback grant exact-source tools only, and stale/invented effects fail closed. | `docs/PI_AGENT_AND_MODEL_PROVIDER_INTEGRATION.md` | P8 | M5 | integration |
| PIGE-PI-004 | BYOK is preset-first: templates bind protocol/Endpoint, Custom Provider exposes compatible protocol, Connect probes before commit, and synced/manual models merge in one refreshable Provider inventory with revision-fenced manual removal. | `docs/PI_AGENT_AND_MODEL_PROVIDER_INTEGRATION.md` | P3 | M3 | integration |
| PIGE-PI-005 | Every Home/source submission creates one durable Pi Job; source preservation is its first checkpoint, after which Pi may answer directly or use authorized tools. Pi's final assistant message is authoritative without a mandatory terminal/grounding/citation shape; tool inputs and durable writes retain owner validation, including one recovered same-page update-plus-tags `update_page` Operation. | `docs/PI_AGENT_AND_MODEL_PROVIDER_INTEGRATION.md` | P3 | M3 | integration |
...
| PIGE-SETTINGS-002 | Machine-local settings and secrets are excluded from default vault backups and are not required to understand restored Markdown knowledge. | `docs/SETTINGS_AND_PREFERENCES.md` | P1 | M1 | integration |
| PIGE-SETTINGS-003 | Sensitive settings change only through explicit user action and their concrete confirmation; untrusted inputs cannot directly change them. | `docs/SETTINGS_AND_PREFERENCES.md` | P1 | M1 | integration |
...
| PIGE-INGEST-003 | v0.1 locally ingests bounded web snapshots plus AnyDoc-owned semantic PDF/DOCX/PPTX content into checksummed artifacts without losing the preserved source; Pige retains only required page/slide/media provenance bridges, and confirmed changed referenced originals refresh only their stable source/page/artifact projection. | `docs/PARSER_INGEST_SPEC.md` | P5 | M4 | integration |
| PIGE-INGEST-004 | OCR routing prefers supported Apple Vision or Windows AI adapters and uses explicit PaddleOCR fallback for unsupported platforms and selected rendered visual-only PDF/PPTX content, with visible retryable unavailability. | `docs/PARSER_INGEST_SPEC.md` | P5 | M4 | integration |
...
| PIGE-EVAL-004 | Retrieval, linking, and summarization quality use executable ranking, grounding, citation, related-page, and insufficient-evidence regression fixtures; Home fails closed on unknown refs and exact Source/current-note/Dataset evidence answers without a Host-owned citation. | `docs/QUALITY_AND_TEST_STRATEGY.md` | P6 | M5 | eval |
...
| PIGE-BACKUP-003 | Backup creation writes a validated versioned `.pige-backup.zip` plus a checksummed manifest that records include/exclude decisions and external dependencies, then reports pathless required-dependency completeness. | `docs/DATA_ARCHITECTURE.md` | P9 | M6 | integration |
...
| PIGE-REL-001 | Phase 1 publishes one protected signed/notarized macOS alpha and supports explicit body-free check, download and restart/apply through the immutable alpha feed; Windows qualification remains next-phase. | `docs/RELEASE_ENGINEERING.md` | P9 | M7 | release |
...
| PIGE-REL-005 | Phase 1 Public Alpha macOS artifacts are Developer ID signed, hardened, notarized, and stapled with release notes and dependency/license attribution. Windows keeps the equivalent next-platform gate; unsigned artifacts stay internal and Linux remains deferred. | `docs/RELEASE_ENGINEERING.md` | P9 | M7 | release |
...
| PIGE-DIAG-002 | Support bundles are user-initiated, previewed, redacted by default, cancelable, and written only to a selected local path; a changed or missing private destination may be explicitly rebound through the same durable Job; bounded redacted recent errors may only seed their existing explicit preview. | `docs/DIAGNOSTICS_AND_OBSERVABILITY.md` | P1 | M1 | integration |
...
| PIGE-UI-001 | The note reader renders polished safe Markdown; editing, title rename, and bounded alias management preserve valid frontmatter, stable identity, links, citations, drafts, and IME behavior. | `docs/UI_PROTOTYPE.md` | P6 | M5 | integration |
