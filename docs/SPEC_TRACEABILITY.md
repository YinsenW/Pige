...
| PIGE-KNOW-005 | Knowledge Health deterministically reports broken links, orphans, duplicate topics, and unsourced claims; only exact eligible repairs apply through Operations/Undo, while exceptions preserve report state and no broad rewrite is silent. | `docs/UI_PROTOTYPE.md` | P9 | M6 | integration |
...
| PIGE-JOB-002 | Retry/recovery adopts exact effects, ingress snapshots, reviewed-plan steps, and completed Pi provider continuations without duplicates; started or drifted provider checkpoints fail closed, while cleanup needs terminal/adopted ownership and no reader. | `docs/JOB_OPERATION_AND_RECOVERY.md` | P2 | M2 | integration |
...
| PIGE-JOB-006 | Reference-based history is restart-safe and compactable; bounded vault search projects safe excerpts and reopens an exact matched event without Provider replay or follow-up authority. | `docs/JOB_OPERATION_AND_RECOVERY.md` | P7 | M5 | integration |
...
| PIGE-INGEST-003 | v0.1 locally ingests bounded web snapshots, PDF text with page locators, semantic DOCX content, and best-effort PPTX content into checksummed artifacts without losing the preserved source; confirmed changed referenced originals refresh only their stable source/page/artifact projection. | `docs/PARSER_INGEST_SPEC.md` | P5 | M4 | integration |
| PIGE-INGEST-004 | OCR routing prefers supported Apple Vision or Windows AI adapters and uses explicit PaddleOCR fallback for unsupported platforms and rendered image-only pages. The bounded local PPTX full-slide/vector/chart port is Pi-selected, identity-bound, retryable when unavailable, and never claims coverage for unsupported primitives. | `docs/PARSER_INGEST_SPEC.md` | P5 | M4 | integration |
