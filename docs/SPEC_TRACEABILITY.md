...
| PIGE-DATA-003 | Managed Collections support stable typed fields, editable views, bounded immutable revision history with forward-only restore, user-created read-only snapshot descriptors pinned to one existing checksummed revision/table without copying payload, bounded citation refs and restart adoption/repair, editable relation/lookup/rollup definitions, revalidated read-only navigation from a relation cell to its exact target record, bounded acyclic nested formulas whose numeric values propagate live into dependent formulas, and reversible Agent row/schema/view changes through Activity/Undo; snapshot create/list/open/citation fail closed for sender, vault, revision, table, or descriptor-path drift and introduce no Parquet, DuckDB, parser, or dependency; destructive, external-write, authority, and conflict boundaries remain exceptional. | `docs/DATA_ARCHITECTURE.md` | P7 | M5 | integration |
