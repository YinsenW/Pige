...
| PIGE-VAULT-004 | Durable knowledge and evidence are not permanently deleted by Agent, package, cleanup, reset, or compaction flows; user-confirmed durable deletes are trash-first and operation-recorded. | `docs/DATA_ARCHITECTURE.md` | P9 | M6 | integration |
...
| PIGE-KNOW-003 | Risky relationship changes such as merges, contradictions, supersession, duplicate marking, and broad hierarchy edits require confirmation. | `docs/KNOWLEDGE_MODEL_AND_LINKING.md` | P7 | M5 | integration |
...
| PIGE-KNOW-006 | Agent ingest produces title, summary, tags, topic, entities, related-note suggestions, schema-valid Markdown/source pages, citations, index/log updates, and a deterministic created/updated/skipped/failed/confirmation-needed action summary. | `docs/PRD.md` | P3 | M3 | integration |
...
| PIGE-JOB-003 | Risky changes become durable confirmation proposals, and approved changes create redacted operation records. | `docs/JOB_OPERATION_AND_RECOVERY.md` | P7 | M5 | integration |
...
| PIGE-JOB-005 | Every applied Agent write creates a redacted operation record with job ID, model profile, source IDs, and changed paths. | `docs/JOB_OPERATION_AND_RECOVERY.md` | P7 | M5 | integration |
...
| PIGE-JOB-007 | Home and its timeline expose durable queued, running, waiting-dependency, waiting-permission, failed, completed, and confirmation-needed states with localized redacted summaries and no premature completion claim. | `docs/JOB_OPERATION_AND_RECOVERY.md` | P2 | M2 | integration |
...
| PIGE-SEC-005 | Every external model call obtains a typed allow, confirm, or block egress decision before prompt assembly or credential access; unknown policy state fails safe. | `docs/AGENT_RUNTIME_POLICY_CONTEXT.md` | P3 | M3 | integration |
...
| PIGE-PI-001 | After deterministic source preservation, the supported upstream Pi runtime is the sole semantic orchestrator through one thin Pige adapter and schema-complete Pige tools; product services cannot bypass it or maintain a parallel Agent/provider or format-driven knowledge pipeline. | `docs/PI_AGENT_AND_MODEL_PROVIDER_INTEGRATION.md` | P3 | M3 | integration |
| PIGE-PI-002 | v0.1 exposes one effective default Pi Agent model; Advanced and Fast routing UI is gated on real runtime support. | `docs/PI_AGENT_AND_MODEL_PROVIDER_INTEGRATION.md` | P3 | M3 | integration |
| PIGE-PI-003 | Pi tools and extensions cannot bypass Permission Broker or Pige-owned storage and secret boundaries. | `docs/PI_AGENT_AND_MODEL_PROVIDER_INTEGRATION.md` | P8 | M5 | integration |
| PIGE-PI-004 | BYOK supports OpenAI, Anthropic, OpenAI-compatible, and Anthropic-compatible provider profiles with connection testing, model-list discovery when available, manual model IDs otherwise, and one effective default model. | `docs/PI_AGENT_AND_MODEL_PROVIDER_INTEGRATION.md` | P3 | M3 | integration |
...
| PIGE-EVAL-002 | Low-confidence, missing-evidence, contradictory, or risky Agent outputs create warnings or proposals instead of confident durable writes. | `docs/QUALITY_AND_TEST_STRATEGY.md` | P3 | M3 | eval |
| PIGE-EVAL-003 | Schema-valid, evidence-backed new source pages and simple new notes may auto-apply, while unsafe, contradictory, low-confidence, destructive, or broad changes remain warning- or proposal-gated. | `docs/QUALITY_AND_TEST_STRATEGY.md` | P3 | M3 | eval |
...
| PIGE-SEARCH-001 | Home knowledge retrieval returns ranked notes plus grounded summary, not answer-only chat. | `docs/PRD.md` | P6 | M5 | integration |
...
| PIGE-REL-001 | v0.1 supports GitHub Actions-built alpha artifacts and automatic update checks. | `docs/RELEASE_ENGINEERING.md` | P9 | M7 | release |
...
| PIGE-REL-005 | Supported macOS and Windows artifacts carry release notes, dependency/license attribution, and an explicit signing/notarization result when credentials are available; Linux remains deferred. | `docs/RELEASE_ENGINEERING.md` | P9 | M7 | release |
...
| PIGE-UI-003 | Home remains capture-first with an essential empty state and no mode chips; compact, expanded, and full-screen reading preserve context, and the collapsible sidebar exposes a directly browsable three-level Library tree when available. | `docs/UI_PROTOTYPE.md` | P1 | M1 | integration |
...
| PIGE-UI-005 | Note Agent and selection actions support copy, quote, ask, translate, polish, expand, summarize, and create-note with local-only copy and proposal-gated mutations. | `docs/UI_PROTOTYPE.md` | P6 | M5 | integration |
