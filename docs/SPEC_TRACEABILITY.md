...
| PIGE-JOB-002 | Retry/recovery adopts exact effects, ingress snapshots, reviewed-plan steps, and a matching completed Pi provider continuation without duplicates; started or drifted checkpoints fail closed, and durable assistant/review publication clears provider result/continuation checkpoint data from the completed Job. | `docs/JOB_OPERATION_AND_RECOVERY.md` | P2 | M2 | integration |
...
| PIGE-JOB-007 | Home exposes durable progress/failure/dependency, searchable and status-filtered Activity/Undo, and exceptional needs-attention state without premature completion. | `docs/JOB_OPERATION_AND_RECOVERY.md` | P2 | M2 | integration |
