...
| PIGE-SEC-005 | Connected/selected Provider plus Send authorizes bounded selected context; explicit secrets are stripped, `local_only` and identity drift block, and ordinary/private/bounded-large context has no second approval. | `docs/AGENT_RUNTIME_POLICY_CONTEXT.md` | P3 | M3 | integration |
...
| PIGE-PI-001 | After preservation, upstream Pi is sole semantic orchestrator; Host owns typed capabilities, high-risk authority, evidence validation, durable reliability, and commits without pipelines, correction loops, or answer synthesis. | `docs/PI_AGENT_AND_MODEL_PROVIDER_INTEGRATION.md` | P3 | M3 | integration |
