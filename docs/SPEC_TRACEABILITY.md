...
| PIGE-VAULT-003 | After Send, each accepted local file gets one private Job snapshot; managed copy adopts it, while reference-original keeps provenance and tools read the snapshot. An unavailable referenced original relinks only after exact proof and preserves its stable Source ID. | `docs/SOURCE_STORAGE_STRATEGY.md` | P2 | M2 | integration |
...
| PIGE-JOB-004 | Missing model, tool, path, or runtime dependencies use visible retryable `waiting_dependency` state rather than silent failure or data loss; exact referenced-original reconnect resumes only matching current waits. | `docs/JOB_OPERATION_AND_RECOVERY.md` | P2 | M2 | integration |
