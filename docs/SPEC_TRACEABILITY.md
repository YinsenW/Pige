...
| PIGE-JOB-002 | Job retry/recovery is idempotent: no duplicate source/page/Operation or consumed reviewed-plan effect; uncertain processes re-probe/adopt or fail closed. | `docs/JOB_OPERATION_AND_RECOVERY.md` | P2 | M2 | integration |
...
| PIGE-SEC-003 | Attachments grant exact accepted-SourceRecord authority only; explicit authored ambient intent may separately use precise confirmation/reviewed effects, never attachment/fallback/model inference. | `docs/SECURITY_THREAT_MODEL.md` | P8 | M5 | integration |
