...
| PIGE-CAP-002 | The manifest-owned large-paste boundary stages exact over-limit clipboard payloads with files, preserves each once as a managed source on atomic Send, and keeps only references in conversation history. | `docs/DATA_ARCHITECTURE.md`; `resources/large-paste-boundary.manifest.json` | P2 | M2 | integration |
