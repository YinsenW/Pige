...
| PIGE-PI-003 | Registered Pi tools require owning-service and authority checks; `external_network` admits private targets, while `public_only` permits only fresh strict-Fake-IP hostname results with required/pinned IPv4 and unchanged fetch controls. | `docs/PI_AGENT_AND_MODEL_PROVIDER_INTEGRATION.md` | P8 | M5 | integration |
