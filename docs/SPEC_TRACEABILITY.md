...
| PIGE-SEC-002 | Only the closed high-risk set—irreversible delete, original overwrite, out-of-root write, arbitrary shell/unknown install, credential disclosure, risky edit, or equivalent escalation—requires explicit confirmation. | `docs/SECURITY_THREAT_MODEL.md` | P8 | M5 | integration |
| PIGE-SEC-003 | Submitted-turn authority covers ordinary registered first-party tools; third-party code cannot inherit it and no saved-grant/YOLO mode bypasses high-risk confirmation. | `docs/SECURITY_THREAT_MODEL.md` | P8 | M5 | integration |
...
| PIGE-PI-003 | Pi tools use Pige-owned adapters: ordinary first-party tools inherit submitted-turn authority, high-risk effects confirm, third-party actors stay isolated, and network/path controls remain enforced. | `docs/PI_AGENT_AND_MODEL_PROVIDER_INTEGRATION.md` | P8 | M5 | integration |
