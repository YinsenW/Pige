...
| PIGE-SEC-003 | Text-only tasks retain ordinary authority; source-bound turns admit exact accepted SourceRecords only; ambient tools, third parties and confirmation cannot widen it. | `docs/SECURITY_THREAT_MODEL.md` | P8 | M5 | integration |
...
| PIGE-PI-003 | Source-bound Pi catalogs contain exact-source tools only; stale/ambient calls fail before effect, while text-only high-risk semantics remain. | `docs/PI_AGENT_AND_MODEL_PROVIDER_INTEGRATION.md` | P8 | M5 | integration |
