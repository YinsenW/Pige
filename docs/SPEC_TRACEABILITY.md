...
| PIGE-PI-004 | BYOK profiles bind an explicit endpoint protocol, pass a real Pi generation/tool probe before persistence, expose one effective default binding, and support reviewed presets plus compatible manual endpoints. | `docs/PI_AGENT_AND_MODEL_PROVIDER_INTEGRATION.md` | P3 | M3 | integration |
| PIGE-PI-005 | Every semantic Home turn uses one durable Pi ingress; Pi may answer directly or choose local/source tools, while no-model work waits without Host intent heuristics or silent fallback. | `docs/PI_AGENT_AND_MODEL_PROVIDER_INTEGRATION.md` | P3 | M3 | integration |
