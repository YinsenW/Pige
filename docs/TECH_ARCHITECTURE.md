...
- DOCX/PPTX: bundled AnyDoc semantic Markdown conversion from verified snapshot bytes, with Pige-owned bounded OpenXML preflight, relationship, media, slide-order, and speaker-notes bridges. The planned PPTX full-slide branch is a replaceable Pige OpenXML-to-pixel port, not an external Office converter.
...
- The verified bounded Swift helper currently serves direct images, Pi-selected PDF pages,
  and selected PPTX media through one schema-versioned, local-only process boundary. The
  separately frozen PPTX full-slide/vector/chart branch stays a Pige-owned bounded worker
  behind the same OCR port; it is not implied by the current selected-media implementation.
...
| `@napi-rs/canvas` (`parser.napi-rs-canvas`) | required | Supply PDF.js Node primitives and bounded native Canvas/PNG encoding for the explicit PDF-page and frozen PPTX full-slide OCR materializer ports. | https://github.com/Brooooooklyn/canvas | Pin `1.0.2`; test native package inclusion and page/slide-renderer startup on every macOS arm64/x64 and Windows x64 release target. | MIT; local-only native dependency; selected-page/slide buffers remain in a bounded worker and are released after transfer. It is not an Office converter. |
...
| Pige OpenXML locator bridge | required | Bounded DOCX/PPTX archive preflight, relationship order, speaker notes, embedded-media locations, and `slide:N` OCR candidates around AnyDoc semantic conversion. | Open XML SDK docs: https://learn.microsoft.com/en-us/office/open-xml/open-xml-sdk | Keep the bridge format-driven and fixture-backed; do not assume slide filenames equal presentation order; full-slide candidates must bind parser-sidecar/current-source identity and the frozen pixel limits. | Does not perform semantic document conversion or bundle LibreOffice in v0.1. |
