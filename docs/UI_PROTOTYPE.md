...
Current Home Activity distinguishes created from updated. Checksum-current exact create/cited append offers non-confirming Undo; changed/hashless/missing fails closed. Settings Activity offers restart-safe Redo after matching Undo for exact user note edit, active-note or Topic-title rename, Library tag lifecycle, exact note merge, Knowledge Health duplicate-topic merge, saved Collection view lifecycle, and Collection cell/row/column/formula/relation/lookup/rollup mutations. Proposals stay transitional; unrelated mutation families, broader restore/redo, broad history, and packaged proof remain open.
...
- Append/replace conflicts retain review, show bounded base/current/proposed lines, and offer
  **Keep current note**, **Apply proposed version**, **Save proposed as a new note**, or
  **Edit manually**. Keep-current is revision-bound and writes no note; apply-proposed records the
  reviewed current bytes for exact Undo before the explicit write. Save-as-new preserves the live
...
- Lightweight labels.
- Auto-generated; browse counts/matches without paths, source IDs, checksums, bodies, or index internals.
...
Knowledge Tree shows growth/connections, not paths; `docs/KNOWLEDGE_MODEL_AND_LINKING.md` owns graph data/review/rebuild. Its sidebar heading, Refresh, totals, and Tree/Network/List views share a body-free aggregate while view/search/focus/camera stay renderer-local. Domain/topic thickness and size show weight; deeper size uses relative density; exact leaf count never depth; fragment-plus-source density uses fixed `0`, `1–2`, `3–5`, `6+` color bands; `needs_review` stays outlined. Meters, detail, and screen-reader text state exact weight/fragment/source/leaf/density so visuals are never sole explanation. Users can collapse or expand a selected branch; Left collapses or moves to its parent, Right expands or enters its first child. Title/type search lists bounded deterministic matches with ancestor paths, reveals hit ancestors, supports Up/Down and Enter-to-open by stable page ID, and restores the prior collapse state when cleared; tree-owner drift clears results without opening Reader. Show all branches clears every local collapse without changing durable knowledge. Bounded outgoing/backlink IDs open confined Reader; drift/failure preserves tree/focus, Back restores focus, synthetic topics stay disabled, opaque IDs read `Source evidence`. Localized loading/ready/empty/degraded/error work; graph editing, manual taxonomy, health workflows, force/advanced analytics remain open.
...
progress/Cancel/Retry, a progressively disclosed ten-item startup-recovery history, and
trash-first clear. Real surfaces include Home,
...
pathless. A compact **Deleted memories** list shows title, kind and deletion time for single deletes,
or only item count and time for a reset; Restore binds the exact current registry and original trash Operation, then adopts the authoritative live and trash
