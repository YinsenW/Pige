...
describe("Agent-selected Dataset ingest tool", () => {
  ...
  it("detects a same-run Dataset child after post-commit parent checkpoint failure", async () => {
    ...
    let pageEffectCode = "";
    const runtime: AgentIngestRuntimePort = {
      run: async (request) => {
        await executeTool(request, "pige_inspect_source", {}, "dataset_checkpoint_inspect");
        try {
          await executeTool(request, "pige_inspect_dataset", {}, "dataset_checkpoint_materialize");
        } catch (caught) {
        ...
        try {
          await executeTool(
            ...
            knowledgeOutput("Must not publish after a committed Dataset child"),
            "publish_after_dataset_checkpoint_failure"
          );
        } catch (caught) {
        ...
        return {
          ...
          events: [],
          assistantText: "The Dataset child committed, so no conflicting page effect was applied.",
    ...
    vi.spyOn(JobExecutionCoordinator.prototype, "markDurableBoundary").mockImplementation(function (
      ...
      if (
        snapshot.job.id === parentId &&
        input.checkpointId === "agent_dataset_child_output_adoption_started"
      ) {
        throw new Error("synthetic parent Dataset adoption checkpoint failure");
      }
    ...
    const parent = readJob(fixture.vaultPath, parentId);
    const child = requireValue(readJobs(fixture.vaultPath).find((job) => job.class === "dataset_import"));
    expect(checkpointFailure).toBe("synthetic parent Dataset adoption checkpoint failure");
    expect(pageEffectCode).toBe("dataset.operation_conflict");
    expect(parent.childJobIds).toEqual([child.id]);
    expect(child).toMatchObject({ state: "completed", parentJobId: parentId });
