...
describe("Home durable Agent conversation UI", () => {
  ...
  it("restores an eligible archived Home-opened note in place and adopts the authoritative active render", async () => {
    const dom = createDom();
    const harness = createHarness(completedGroundedTimeline());
    const revision = `noteeditrev_${"d".repeat(32)}`;
    let mode: "stale" | "committed" = "stale";
    harness.renderNote = async (pageId) => ({
      ...testRenderedNote(pageId),
      summary: { ...testRenderedNote(pageId).summary, status: "archived" },
      restoreEligibility: { canRestore: true, revision }
    });
    harness.restoreArchived = async (request) => mode === "stale"
      ? { ...request, status: "stale" }
      : {
          ...request,
          status: "committed",
          operationId: "operation_home_note_restore",
          render: {
            ...testRenderedNote(request.currentPageId),
            html: "<p>Restored Home note.</p>",
            renderContextId: `notectx_${"e".repeat(32)}`,
            restoreEligibility: { canRestore: false, revision: `noteeditrev_${"e".repeat(32)}` }
          }
        };
    const mount = await mountHome(dom, makePigeApi(harness));
    const citation = requireElement(mount.container.querySelector<HTMLButtonElement>(".conversation-citations .citation-row"));
    ...
    await clickButtonByAriaLabel(dom, mount.container, enMessages["note.moreActions"]);
    await clickButton(dom, mount.container, enMessages["note.document.restore"]);
    await clickButton(dom, mount.container, enMessages["note.document.restoreConfirm"]);
    await waitFor(dom, () => harness.noteRestoreRequests.length === 1);
    expect(harness.noteRestoreRequests[0]).toMatchObject({
      activeVaultId: "vault_home_conversation",
      currentPageId: "page_20260715_note0001",
      renderContextId: `notectx_${"a".repeat(32)}`,
      expectedRevision: revision
    ...
    expect(mount.container.querySelector(".note-reader")).not.toBeNull();
    expect(mount.container.textContent).toContain(enMessages["note.document.restoreFailed"]);
    expect(dom.window.document.activeElement).toBe(buttons(mount.container, enMessages["note.document.restoreCancel"])[0]);

    mode = "committed";
    await clickButton(dom, mount.container, enMessages["note.document.restoreConfirm"]);
    await waitFor(dom, () => mount.container.textContent?.includes("Restored Home note.") === true);
    expect(harness.noteRestoreRequests).toHaveLength(2);
    expect(mount.container.querySelector(".note-reader")).not.toBeNull();
    await waitFor(dom, () => dom.window.document.activeElement === mount.container.querySelector(".note-reader"));
  ...
  it("corrects tags and topics on the exact Home-opened Reader note and retains both drafts on stale", async () => {
    const dom = createDom(); const harness = createHarness(completedGroundedTimeline()); const revision = `noteeditrev_${"d".repeat(32)}`;
    let mode: "stale" | "committed" = "stale";
    harness.renderNote = async (pageId) => ({ ...testRenderedNote(pageId), tagging: { tags: ["research"], topics: ["PKM"], canAdd: true, canEdit: true, revision } });
    harness.editTaxonomy = async (request) => mode === "stale" ? { ...request, status: "stale" } : { ...request, status: "committed",
      operationId: "operation_home_note_taxonomy", render: { ...testRenderedNote(request.currentPageId), html: "<p>Classified Home note.</p>",
        tagging: { tags: [...request.tags], topics: [...request.topics], canAdd: true, canEdit: true, revision: `noteeditrev_${"e".repeat(32)}` } } };
    const mount = await mountHome(dom, makePigeApi(harness));
    ...
    await waitFor(dom, () => mount.container.querySelector(".note-reader") !== null);
    await clickButtonByAriaLabel(dom, mount.container, enMessages["note.moreActions"]); await clickButton(dom, mount.container, enMessages["note.taxonomy.edit"]);
    const tagsInput = requireElement(mount.container.querySelector<HTMLInputElement>(`input[placeholder="${enMessages["note.taxonomy.tagsPlaceholder"]}"]`));
    const topicsInput = requireElement(mount.container.querySelector<HTMLInputElement>(`input[placeholder="${enMessages["note.taxonomy.topicsPlaceholder"]}"]`));
    await act(async () => { setInput(dom, tagsInput, "  field   notes , research  "); setInput(dom, topicsInput, "PKM, Knowledge   management"); await settle(dom); });
    await clickButton(dom, mount.container, enMessages["note.taxonomy.confirm"]);
    await waitFor(dom, () => harness.noteEditTaxonomyRequests.length === 1);
    expect(harness.noteEditTaxonomyRequests[0]).toMatchObject({ activeVaultId: "vault_home_conversation", currentPageId: "page_20260715_note0001",
      renderContextId: `notectx_${"a".repeat(32)}`, expectedRevision: revision, tags: ["field notes", "research"], topics: ["PKM", "Knowledge management"] });
    expect(mount.container.textContent).toContain(enMessages["note.taxonomy.failed"]); expect(tagsInput.value).toBe("  field   notes , research  ");
    expect(topicsInput.value).toBe("PKM, Knowledge   management"); expect(dom.window.document.activeElement).toBe(tagsInput);
    mode = "committed"; await clickButton(dom, mount.container, enMessages["note.taxonomy.confirm"]);
    await waitFor(dom, () => mount.container.textContent?.includes("Classified Home note.") === true);
    expect(harness.noteEditTaxonomyRequests).toHaveLength(2); expect(mount.container.querySelector(".note-reader")).not.toBeNull();
    await waitFor(dom, () => dom.window.document.activeElement === mount.container.querySelector(".note-reader"));
...
function makePigeApi(harness: ConversationHarness): object {
  return {
    ...
    notes: {
      ...
      editTaxonomy: async (request: NoteEditTaxonomyRequest) => {
