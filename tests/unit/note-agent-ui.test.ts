...
describe("Note Agent production UI", () => {
  ...
  it("keeps the UI adapter service-free, responsive, and localized in all six catalogs", () => {
    ...
    expect(appSource).toContain('key={`${activeVault.vaultId}:${noteAgentContext.pageId}:${noteAgentExternalRevision}`}');
    expect(appSource).toContain("vaultId={activeVault.vaultId}");
    expect(appSource).toContain("selectedNoteVaultId === activeVault.vaultId");
    expect(appSource).toContain("pageId={noteAgentContext.pageId}");
    ...
    expect(appSource).toContain('decision: action === "apply" ? "approve" : action === "reject" ? "reject" : action');
    expect(appSource).toContain("expectedCurrentRevision: current.preview.currentRevision");
    expect(appSource).toContain("readerSelectionCreatedPageType(result.proposal.action)");
    expect(appSource).toContain('? await openNoteTarget(result.createdPageId, false, createdPageType ?? "note")');
