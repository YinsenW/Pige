...
describe("NoteAliasService", () => {
  it("adds and removes one unique alias through durable Markdown with Activity, Undo, and restart", async () => {
    ...
    write(fixture, "wiki/current.md", noteMarkdown("Primary Name", ["Second Name"]));
...
function makeService(fixture: ReturnType<typeof makeFixture>, editor: Pick<NoteMarkdownEditorService, "open" | "save">, current = true) {
  return new NoteAliasService({ resolveTrashTarget: vi.fn(() => { const markdown = read(fixture, "wiki/current.md"); return { status: "ready",
    ...
    render: vi.fn(async () => { const markdown = read(fixture, "wiki/current.md"), aliases = parsePigeFrontmatter(markdown)?.frontmatter.aliases ?? [];
      return { summary: { pageId, title: "Primary Name", pageType: "note" as const, status: "active" as const, pagePath: "wiki/current.md",
...
function noteMarkdown(title: string, aliases: readonly string[], id = pageId) { return `---\nid: ${JSON.stringify(id)}\nschema_version: 1\ntitle: ${JSON.stringify(title)}\ntype: "note"\ncreated_at: "2026-07-31T10:00:00.000Z"\nupdated_at: "2026-07-31T10:00:00.000Z"\nstatus: "active"\naliases: ${JSON.stringify(aliases)}\nsource_ids: []\n---\n\n# ${title}\n\nKeep body.\n`; }
