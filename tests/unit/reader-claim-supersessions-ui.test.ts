...
import { ReaderClaimSupersessions } from "../../apps/desktop/src/renderer/src/components/ReaderClaimSupersessions";
const keys = ["window", "document", "navigator", "Node", "HTMLElement", "HTMLInputElement", "Event", "InputEvent", "requestAnimationFrame"] as const, originals = new Map<PropertyKey, PropertyDescriptor | undefined>();
afterEach(() => { for (const key of keys) { const value = originals.get(key); if (value) Object.defineProperty(globalThis, key, value); else Reflect.deleteProperty(globalThis, key); } originals.clear(); Reflect.deleteProperty(globalThis, "IS_REACT_ACT_ENVIRONMENT"); });
describe("ReaderClaimSupersessions", () => {
  it("requires confirmation before it adopts an authoritative directed supersession render", async () => {
    ...
    expect(change).toHaveBeenCalledWith(expect.objectContaining({ action: "add", targetPageId: item().pageId, expectedTargetUpdatedAt: item().updatedAt })); expect(committed).toHaveBeenCalledWith(render([item()], "b")); await harness.unmount();
  });
...
function render(items: readonly ReturnType<typeof item>[] = [], revision = "a"): NoteRenderResult { return { summary: { pageId: "page_20260808_claim0001", title: "Claim", pageType: "claim", status: "active", pagePath: "wiki/claim.md", createdAt: "2026-08-08T10:00:00.000Z", updatedAt: "2026-08-08T10:00:00.000Z", sourceIds: ["src_20260808_claimbase"] }, html: "<h1>Claim</h1>", byteSize: 64, renderContextId: `notectx_${revision.repeat(32)}`, claimSupersessions: { canEdit: true, revision: `noteeditrev_${revision.repeat(64)}`, items } }; }
function item() { return { pageId: "page_20260808_claim0002", title: "Prior claim", updatedAt: "2026-08-08T11:00:00.000Z" }; }
