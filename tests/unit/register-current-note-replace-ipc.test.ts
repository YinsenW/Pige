...
describe("registerCurrentNoteReplaceIpc", () => {
  ...
  it("keeps a conflict in durable review until the exact current revision is accepted", async () => {
    const handlers = new Map<string, (_event: unknown, request: unknown) => unknown>();
    const currentRevision = `noteeditrev_${"b".repeat(64)}` as const;
    const conflicted = { ...replacePreview(), state: "conflicted" as const, revision: 3, currentRevision };
    const rejected = { ...replacePreview(), state: "rejected" as const, revision: 4 };
    ...
    registerCurrentNoteReplaceIpc({
      ...
      currentVault: () => ({ vaultId: VAULT_ID } as VaultSummary),
      activeVaultPath: () => "/synthetic/vault",
      getService: () => ({ getProposal: () => conflicted, decideProposal } as unknown as CurrentNoteReplaceService),
    ...
    const kept = await handlers.get("agent.decideCurrentNoteReplaceProposal")?.({}, {
      apiVersion: 1,
      activeVaultId: VAULT_ID,
      jobId: JOB_ID,
      proposalId: PROPOSAL_ID,
